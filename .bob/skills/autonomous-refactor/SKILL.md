...
description: >-
  Master autonomous refactoring orchestrator. Runs nested loops - outer loop
  executes EPICs sequentially via individual phase commands, inner loop drives
  each PR to 100/100 via /pr-loop. Continues until entire codebase reaches CYC ≤
  8 with Jane Street compliance.
metadata:
  user-invocable: true
