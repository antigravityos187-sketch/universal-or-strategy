...
if [ ! -f "docs/brain/EPIC-W7-033/00-hotspots.md" ]; then
    echo "BLOCKED: Phase 0 not complete"
    exit 1
fi

cat > /tmp/phase1_msg_033.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition) for EPIC-W7-033.
...
3. Write docs/brain/EPIC-W7-033/00-scope.md using execute_command
...
bob --yolo --chat-mode v12-phase1-scope "$(cat /tmp/phase1_msg_033.txt)" 2>&1 | tee logs/wave7/phase1/EPIC-W7-033.log
echo "DONE_EXIT=$?"
