...
## 3.5. Real-Time Code Quality Monitoring (V12.22)

**MANDATORY**: All agents MUST check VSCode panels before EVERY commit.

### Pre-Commit Panel Review Protocol

**Before ANY `git commit`**, you MUST:

1. **Check Bob Findings Panel**:
   - Open: View → Problems → Filter: "Bob Findings"
   - Filter to files you modified
   - **BLOCKING**: Zero P0 (Critical) findings allowed
   - **REQUIRED**: Document P1 (High) findings as technical debt
   - **OPTIONAL**: P2 (Medium) findings can be deferred

2. **Check Problems Panel**:
   - Open: View → Problems
   - Filter to files you modified
   - **BLOCKING**: Zero errors allowed (warnings are informational)
   - Fix all compilation errors before commit

3. **Check Comments Panel** (if working on existing PR):
   - Open: View → Comments
   - Review any new bot comments on your changes
   - Address P0/P1 issues before commit

### Real-Time Workflow Integration

**During Coding:**
- Bob Findings update as you type
- Use "Fix with Bob" button for automated refactoring
- Problems panel shows linter issues immediately

**Before Committing:**
```powershell
# 1. Visual panel check (see above)
...
**Rationale:** Catching issues before commit prevents:
- Failed CI builds
- PR review churn
- Wasted bot analysis cycles
