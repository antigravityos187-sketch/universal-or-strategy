...
cat > /tmp/phase5_msg_$EPIC_ID.txt << 'EOFMSG'
Execute ticket $TICKET_ID for $EPIC_ID based on tickets file $TICKETS_FILE. Target CYC ≤8 (Jane Street strict). Generate xUnit tests. Output: $OUTPUT_FILE
EOFMSG

# Step 3.2: Execute Bob CLI with command substitution
~/.npm-global/bin/bob --mode v12-engineer --yolo "$(cat /tmp/phase5_msg_$EPIC_ID.txt)" 2>&1 | tee "logs/wave6/phase5/$EPIC_ID-ticket-$TICKET_ID.log"
