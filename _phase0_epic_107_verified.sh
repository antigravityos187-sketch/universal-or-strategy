...
# Set API key
export BOB_API_KEY_FILE="$HOME/.bob/api-keys/b (2).json"
...
bob --mode advanced \
    --message "Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-107.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='ProcessIpcCommands')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='ProcessIpcCommands')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='ProcessIpcCommands')

### Step 2: Write 00-hotspots.md
Use write_to_file tool to create docs/brain/EPIC-CCN-107/00-hotspots.md with:
...
Use write_to_file tool to create docs/brain/EPIC-CCN-107/manifest.json:
```json
{
...
1. read_file docs/brain/EPIC-CCN-107/00-hotspots.md
2. read_file docs/brain/EPIC-CCN-107/manifest.json
