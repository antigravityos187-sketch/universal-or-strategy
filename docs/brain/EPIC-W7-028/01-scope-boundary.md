# EPIC-W7-028 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-028/00-scope.md
...
**`ProcessFlattenWorkItem_CancelOrders` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ProcessFlattenWorkItem_CancelOrders` in `src/V12_002.SIMA.Flatten.cs`
...
- `FlattenWorkItem` — confirmed not modified by this epic
- `PumpFlattenOps` — confirmed not modified by this epic
- `PerformFallbackFlatten` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `ProcessFlattenWorkItem_CancelOrders` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `ProcessFlattenWorkItem_CancelOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-028/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ProcessFlattenWorkItem_CancelOrders` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.SIMA.Flatten.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The scope boundary for this epic is `ProcessFlattenWorkItem_CancelOrders` in its entirety (lines 191–238 of `src/V12_002.SIMA.Flatten.cs`). Everything outside that boundary — including its callers, th
