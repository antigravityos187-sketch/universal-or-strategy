# EPIC-W7-143 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-143/00-scope.md
...
**`OnKeyDown` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `OnKeyDown` in `src/V12_002.UI.Callbacks.cs`
...
is internal to `OnKeyDown` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 44 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `OnKeyDown`
...
- **Source analyzed:** `docs/brain/EPIC-W7-143/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `OnKeyDown` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `OnKeyDown` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `OnKeyDown` in `src/V12_002.UI.Callbacks.cs`.
No other me
