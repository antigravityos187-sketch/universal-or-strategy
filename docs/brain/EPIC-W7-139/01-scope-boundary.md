# EPIC-W7-139 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-139/00-scope.md
...
**`UpdateStopOrder` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `UpdateStopOrder` in `src/V12_002.Trailing.StopUpdate.cs`
...
Direct caller count (from 00-scope.md analysis): **25**
...
is internal to `UpdateStopOrder` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 1 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `UpdateStopOrder`
...
- **Source analyzed:** `docs/brain/EPIC-W7-139/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `UpdateStopOrder` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 25, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Trailing.StopUpdate.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `UpdateStopOrder` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `UpdateStopOrder` in `src/V12_002.Trailing.StopUpda
