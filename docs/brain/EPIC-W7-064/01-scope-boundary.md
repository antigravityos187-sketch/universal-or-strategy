# EPIC-W7-064 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-064/00-scope.md
...
**`ResolveFsm_ByScan` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ResolveFsm_ByScan` in `src/V12_002.Symmetry.BracketFSM.cs`
...
- `ResolveFsmFromEvent` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `ResolveFsm_ByScan` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `ResolveFsm_ByScan`
...
- **Source analyzed:** `docs/brain/EPIC-W7-064/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ResolveFsm_ByScan` present and in-scope per Phase 1 analysis
