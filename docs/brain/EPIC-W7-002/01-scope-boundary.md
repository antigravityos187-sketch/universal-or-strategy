# EPIC-W7-002 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-002/00-scope.md
...
**`SymmetryGuardTryResolveFollowersForDispatch` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SymmetryGuardTryResolveFollowersForDispatch` in `src/V12_002.Symmetry.Replace.cs`
...
is internal to `SymmetryGuardTryResolveFollowersForDispatch` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `SymmetryGuardTryResolveFollowersForDispatch`
...
- **Source analyzed:** `docs/brain/EPIC-W7-002/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SymmetryGuardTryResolveFollowersForDispatch` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **scope boundary**: Only `SymmetryGuardTryResolveFollowersForDispatch` (lines 134–191 of
