# EPIC-W7-036 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-036/00-scope.md
...
**`MoveStop_SinglePosition` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `MoveStop_SinglePosition` in `src/V12_002.Trailing.Breakeven.cs`
...
Direct caller count (from 00-scope.md analysis): **59**

- `MoveStopsToBreakevenWithOffset` — confirmed not modified by this epic
- `TryHandleBreakeven` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `MoveStop_SinglePosition` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `MoveStop_SinglePosition`
...
- **Source analyzed:** `docs/brain/EPIC-W7-036/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `MoveStop_SinglePosition` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 59, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Trailing.Breakeven.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for this phase is defined as follows:

- **In scope:** The body of `MoveStop_SinglePosition` (lines 73–163 of
  `src/V12_002.Trailing.Breakeven.cs`) and the three private helper
