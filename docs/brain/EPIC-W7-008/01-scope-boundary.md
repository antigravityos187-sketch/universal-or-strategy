# EPIC-W7-008 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-008/00-scope.md
...
**`ManageCIT` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ManageCIT` in `V12_002.Orders.Management.Flatten.cs`
...
is internal to `ManageCIT` only — extracting sub-logic into private helpers without
...
- **Target file:** `V12_002.Orders.Management.Flatten.cs`
...
are unaffected. The blast radius is contained to the single source file where `ManageCIT`
...
- **Source analyzed:** `docs/brain/EPIC-W7-008/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ManageCIT` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 12, all confirmed upstream-only
- **Blast radius:** Limited to `V12_002.Orders.Management.Flatten.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for EPIC-W7-008 is precisely the `ManageCIT` method cluster: the method body (lines 68–128) and the five private helper methods it exclusively calls (`ValidateCitConfiguration`,
