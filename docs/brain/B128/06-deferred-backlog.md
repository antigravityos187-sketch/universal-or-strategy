...
|------|-------------|-----------|
| B128-REQ-01 | `_instrRowPanel` (UniformGrid, 2 cols) inserted above `_quickRowPanel` in `root.Children` | B128-T1 CHANGE 2 |
| B128-REQ-02 | QX-Instr button with spinner arrows; default `_instrQxT1=4`; `FormatBuffer("QX-Instr", _instrQxT1)` | B128-T1 CHANGE 1+3 |
| B128-REQ-03 | `ComputeInstrSplit(int)` internal static; `t1=(n+1)/2`, `t2=n/2` | B128-T1 CHANGE 4 |
| B128-REQ-04 | `OnInstrQxClick` calls `ComputeInstrSplit` then `PttQuickExit.Execute(_leaderAccount, _instrument, t1, t2)` | B128-T1 CHANGE 5 |
| B128-REQ-05 | BE-Instr button; `OnInstrBeClick` calls `_engine.ArmPendingBe(_instrument, _leaderAccount, _beBuffer)` | B128-T1 CHANGE 7 |
| B128-REQ-06 | Log `"[PTT-QX-INSTR]"` on QX click, `"[PTT-BE-INSTR]"` on BE click | B128-T1 CHANGE 5+7 |
| B128-REQ-07 | 4 xUnit `[Fact]` tests in `Tests/B128Tests.cs` covering `ComputeInstrSplit(4,5,1,7)` | B128-T1 CHANGE 8+9 |

---

## New Deferred Items — B128

### DW-B128-01 — Director SIM Gate: QX-Instr + BE-Instr Live Validation

**Priority**: P1 — required before using QX-Instr / BE-Instr in a live session
**Context**: The `_instrQxBtn` and `_instrBeBtn` buttons added in B128 have not been exercised
in a live NT8 SIM session. The implementation is code-verified and build-clean, but the
following behavioral assertions require a Director SIM session to confirm:
- `_instrument` field resolves non-null on a chart that has an instrument loaded.
- `TryResolveLeaderAccount()` returns a non-null leader account when copier is configured.
- `[PTT-QX-INSTR]` log line appears in Output tab with correct account/instrument/t1/t2 values when QX-Instr is clicked.
- `PttQuickExit.Execute(_leaderAccount, _instrument, t1, t2)` fires and submits PTT-QX-* brackets for the current instrument only (not global sweep).
- `[PTT-BE-INSTR]` log line appears in Output tab with correct account/instrument/buf values when BE-Instr is clicked.
- `_engine.ArmPendingBe(_instrument, _leaderAccount, _beBuffer)` arms the pending BE watcher for the per-instrument scope.
- No naked positions result from either button action.

**Test sequence (recommended)**:
1. Open TradeCopierPanel on a chart with NQ/ES/MES loaded.
2. Start Copier with leader + 1 follower.
3. Enter a small test position.
4. Click QX-Instr — confirm Output tab shows `[PTT-QX-INSTR] button: {acc} {instr} t1=2 t2=2` (default T1=4 → (2,2)).
5. Click BE-Instr — confirm Output tab shows `[PTT-BE-INSTR] button: {acc} {instr} buf=1`.
6. Confirm no unprotected positions remain.

**Deferred to**: B129 or first SIM gate session after B128 sync.

---

## Carry-Forward Items (from B124 — status unchanged)

All items below are copied verbatim from `docs/brain/B124/06-deferred-backlog.md`.
B128 changes do not affect any of these items.
...
CS0433 Globals ambiguity at CopyEngine.cs:L3350. Confirmed pre-existing, unrelated to B128.
...
|----------|-------|-------|
| Closed this block | 7 | B128-REQ-01, B128-REQ-02, B128-REQ-03, B128-REQ-04, B128-REQ-05, B128-REQ-06, B128-REQ-07 |
| New deferred (B128) | 1 | DW-B128-01 (Director SIM gate: QX-Instr + BE-Instr live validation) |
| Carry-forward from B124 (unchanged) | 16 | DW-B124-01/02, DW-B107, B107-DEFER-01/02, DW-B42-01/02/03, DW-PTT-BE-FIX-01/02/03, DW-B89-DEFERRED-01/02/03/04/05/06 |

**Total open items**: 17 (1 new + 16 carry-forward)

B128 changes do not affect any carry-forward items from B124.
