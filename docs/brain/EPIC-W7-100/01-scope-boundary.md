# EPIC-W7-100 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-100/00-scope.md
...
**`ClosePositionsOnlyApexAccounts` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ClosePositionsOnlyApexAccounts` in `src/V12_002.SIMA.Flatten.cs`
...
is internal to `ClosePositionsOnlyApexAccounts` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 106 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `ClosePositionsOnlyApexAccounts`
...
- **Source analyzed:** `docs/brain/EPIC-W7-100/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ClosePositionsOnlyApexAccounts` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `ClosePositionsOnlyApexAccounts` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ClosePositionsOnlyApexAccounts` in 
