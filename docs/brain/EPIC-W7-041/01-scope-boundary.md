# EPIC-W7-041 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-041/00-scope.md
...
**`AuditStopQuantityAndPrint` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `AuditStopQuantityAndPrint` in `src/V12_002.Orders.Management.cs`
...
- `SubmitBracketOrders` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `AuditStopQuantityAndPrint` only — extracting sub-logic into private helpers without
...
- **Target file:** `src/V12_002.Orders.Management.cs`
...
are unaffected. The blast radius is contained to the single source file where `AuditStopQuantityAndPrint`
...
- **Source analyzed:** `docs/brain/EPIC-W7-041/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `AuditStopQuantityAndPrint` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Orders.Management.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for this epic is strictly the body of the single method
`AuditStopQuantityAndPrint` (lines 90–174, `src/V12_002.Orders.Management.cs`).
All work — branch decomposition, extracti
