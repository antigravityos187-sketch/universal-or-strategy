# EPIC-W7-111 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-111/00-scope.md
...
**`HydrateExpectedPositionsFromBroker` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `HydrateExpectedPositionsFromBroker` in `src/V12_002.SIMA.Lifecycle.cs`
...
is internal to `HydrateExpectedPositionsFromBroker` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 2 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `HydrateExpectedPositionsFromBroker`
...
- **Source analyzed:** `docs/brain/EPIC-W7-111/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `HydrateExpectedPositionsFromBroker` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `HydrateExpectedPositionsFromBroker` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `HydrateExpectedPositionsFromBro
