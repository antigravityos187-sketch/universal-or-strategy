# EPIC-W7-049 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-049/00-scope.md
...
**`ManageTrail_RunPerTradeBranches` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ManageTrail_RunPerTradeBranches` in `src/V12_002.Trailing.cs`
...
- `ManageTrailingStops` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `ManageTrail_RunPerTradeBranches` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 11 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `ManageTrail_RunPerTradeBranches`
...
- **Source analyzed:** `docs/brain/EPIC-W7-049/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ManageTrail_RunPerTradeBranches` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 2, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Trailing.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for EPIC-W7-049 is the body of `ManageTrail_RunPerTradeBranches`
