# EPIC-W7-014 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-014/00-scope.md
...
**`TryHandleFleetCommand` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `TryHandleFleetCommand` in `src/V12_002.UI.IPC.Commands.Fleet.cs`
...
Direct caller count (from 00-scope.md analysis): **4**

- `ProcessIpcCommandCore` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `TryHandleFleetCommand` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 37 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `TryHandleFleetCommand`
...
- **Source analyzed:** `docs/brain/EPIC-W7-014/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `TryHandleFleetCommand` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 4, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.UI.IPC.Commands.Fleet.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for this epic is drawn at the dispatcher body of `TryHandleFleetCommand`
(lines 37–81) only. The boundary is defined as:

- **In scope:** the `cmdId` ternary construction (lines
