...
> **Only `ProcessShutdownSIMA` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ProcessShutdownSIMA` in `src/V12_002.SIMA.Lifecycle.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `ProcessShutdownSIMA` must remain
...
**Thought 1**: `ProcessShutdownSIMA` in `src/V12_002.SIMA.Lifecycle.cs` has CYC=3, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `ProcessShutdownSIMA` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `ProcessShutdownSIMA` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `ProcessShutdownSIMA`
...
| **Output** | docs/brain/EPIC-W7-114/00-scope.md |
