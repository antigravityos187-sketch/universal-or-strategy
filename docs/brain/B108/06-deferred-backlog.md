# B108 Deferred Backlog

Block: B108 (DW-B107 Fix — SnapshotBeTargets + Cap-at-3)
Date: 2026-08-11
Status: PIPELINE_COMPLETE (VERIFY_PASS received, F5 gate pending Director)

---

## Block B108 — DW-B107 Fix (SnapshotBeTargets + Cap-at-3)

**Pipeline status**: FINAL_PASS (VERIFY_PASS received, F5 gate pending Director)
**Date**: 2026-08-11
**Tickets**: B108-T1 (1 ticket, 1 file, 3 changes)

### Deferred Items

| ID | Item | Reason | Target Block |
|----|------|--------|--------------|
| DW-B108-D01 | F5 NinjaTrader 8 compile confirmation | Must be done by Director in live NT8 instance after sync | B108 close (Director action) |
| DW-B108-D02 | xUnit test: MoveStopToBreakEven stale residue regression test | Pre-existing test coverage gap; not introduced by B108; low risk (structural refactor preserves all logic) | Future testing block |

### Closed Items

| ID | Item | Closed By |
|----|------|-----------|
| DW-B107 | Stale PTT-BE-Target-* residues in MoveStopToBreakEven Step A | B108-T1 (SnapshotBeTargets + cap-at-3) |

---

## Items Carried Forward from B107 (unchanged)

Items below are copied from `docs/brain/B107/06-deferred-backlog.md`.
B108 changes do not affect any of these items.
...
The F5 NinjaTrader 8 compilation step is the runtime compile gate for B107 changes. It must
produce "Compilation succeeded" (zero errors) after sync. This is Director-owned.
...
CS0433 Globals ambiguity at CopyEngine.cs:L3350. Confirmed pre-existing, unrelated to B107/B108.
...
|----------|-------|-------|
| Closed this block (B108) | 1 | DW-B107 |
| New deferred (B108 pipeline) | 2 | DW-B108-D01 (F5), DW-B108-D02 (regression test) |
| Carry-forward from B107 (unchanged) | 13 | B107-DEFER-01/02, DW-B42-01/02/03, DW-PTT-BE-FIX-01/02/03, DW-B89-DEFERRED-01/02/03/04/05/06 |

**Total open items**: 15 (2 new + 13 carry-forward)
