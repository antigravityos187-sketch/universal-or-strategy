...
**Thought 1**: `Dispatch_ProcessFleetLoop` in `src/V12_002.SIMA.Dispatch.cs` has CYC=14, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `Dispatch_ProcessFleetLoop` only -- no scope creep.
