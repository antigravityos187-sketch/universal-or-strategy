# EPIC-W7-161 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-161/00-scope.md
...
**`SyncLiveTargetRows` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SyncLiveTargetRows` in `src/V12_002.UI.Panel.StateSync.cs`
...
is internal to `SyncLiveTargetRows` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `SyncLiveTargetRows`
...
- **Source analyzed:** `docs/brain/EPIC-W7-161/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SyncLiveTargetRows` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `SyncLiveTargetRows` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `SyncLiveTargetRows` in `src/V12_002.UI.Panel.St
