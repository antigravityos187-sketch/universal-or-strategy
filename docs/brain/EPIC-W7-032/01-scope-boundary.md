# EPIC-W7-032 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-032/00-scope.md
...
**`RestoreCascadedTargets` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `RestoreCascadedTargets` in `src/V12_002.Orders.Management.StopSync.cs`
...
- `GetTargetOrdersDictionary` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `RestoreCascadedTargets` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 23 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `RestoreCascadedTargets`
...
- **Source analyzed:** `docs/brain/EPIC-W7-032/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `RestoreCascadedTargets` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 3, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Orders.Management.StopSync.cs` + private helpers (same partial class)
