# EPIC-W7-052 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-052/00-scope.md
...
**`CleanupStalePendingReplacements` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `CleanupStalePendingReplacements` in `src/V12_002.Trailing.StopUpdate.cs`
...
is internal to `CleanupStalePendingReplacements` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `CleanupStalePendingReplacements`
...
- **Source analyzed:** `docs/brain/EPIC-W7-052/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `CleanupStalePendingReplacements` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** The scope boundary for EPIC-W7-052 is strictly limited to:
