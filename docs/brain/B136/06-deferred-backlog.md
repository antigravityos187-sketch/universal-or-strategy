# B136 Deferred Backlog

**Block**: B136
**Block Header**: B136 Deferred Items
**Produced by**: ptt-plan-reviewer (Phase 5)
**Prior backlog**: `docs/brain/B135/06-deferred-backlog.md`
**Date**: 2026-09-07

---

## Status Changes From B135

| ID | B135 Status | B136 Status | Change |
|----|-------------|-------------|--------|
| DW-B148 | OPEN | **CLOSED** | B136-T1 implemented and VERIFY_PASS issued. OrderPassesBracketGate fused guard deployed; 9/9 B136 tests PASS; fix path verified end-to-end in source. |
| DW-B146 | OPEN | **CLOSED** | Consequential closure. MatchesLeaderName is now reachable on the ATM path via OrderPassesBracketGate. No independent code change required. |
| DW-B147 | DEFERRED (P2) | DEFERRED (P2) | No change. |
| DW-B141 | OPEN (awaiting SIM Test A) | OPEN (awaiting SIM Test A) | No change. SIM Test A not yet run. |
...
**Description**: `SyncAtmFollowerBracket` (CYC=4) and `SyncAtmFollowerTarget` (CYC=8, AT LIMIT)
both execute a cancel+resubmit sequence even when `rawPrice == newPrice` (i.e., no price change
occurred on the leader side). Adding a no-op early-return guard (`if (rawPrice == newPrice) return;`)
would eliminate redundant order cancel/resubmit cycles.
...
**Resolution Condition**: Future block (B137+) extracts `IsNoPriceChange` helper from
`SyncAtmFollowerTarget` (reducing it from CYC=8), then applies the guard to both methods cleanly.

**Reference**: B135 plan §E; B136 05-final-review.md Section K.
...
replacement code. The B136 fix further strengthens the prerequisite by ensuring `fo ≠ null` on
the second drag.
...
**Resolution Condition**: Director or engineer runs SIM Test A with B136 fix deployed; reviewer
marks CLOSED upon confirmed Phase C stop sync.
...
The B136 fix ensures PTT-STP-Drag is reachable via the fused `OrderPassesBracketGate` on the ATM path.
...
**Resolution Condition**: Director runs SIM Test B with B136 fix deployed; confirms follower stop
syncs correctly.
...
empirically across a reconnect cycle. The B136 `OrderPassesBracketGate` fix does not change this
exposure — it only changes how matching is performed, not the order set iterated.
...
**Remaining open sub-observations** (NOT addressed by T2 sweep or B136):
...
| DW-B148 | B136 | B136-T1: OrderPassesBracketGate fused guard deployed. VERIFY_PASS issued. 9/9 B136 tests PASS. Fix path verified end-to-end: signalName=null → MatchesLeaderName → PTT-TGT-Drag/PTT-STP-Drag → true. |
| DW-B146 | B136 | Consequential closure via DW-B148. MatchesLeaderName now reachable for all ATM-path PTT-prefix replacement orders. Second drag fo=null condition eliminated. |
...
|----|-------|----------|--------------|--------|
| DW-B148 | SignalOrNameMatches PTT-prefix gate | P1 | B136 | **CLOSED** |
| DW-B146 | Second drag fo=null | P1 | B136 | **CLOSED** |
| DW-B147 | rawPrice==newPrice early-return guard | P2 | B137+ | DEFERRED |
...
| B135-DEFER-01 | Gap B -- two simultaneous entries | P1 | B137+ | OPEN |
...
*2 items CLOSED this block (DW-B148, DW-B146). 6 items remain open/deferred. All require director
SIM data or future block implementation. DW-B134-OCO (main) closed by B135 T2 (carried for history).*

---

*Produced by ptt-plan-reviewer, B136 Phase 5. Required gate artifact for FINAL_PASS.*
