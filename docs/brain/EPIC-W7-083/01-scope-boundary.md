# EPIC-W7-083 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-083/00-scope.md
...
**`AuditMaster_CheckExpectedActual` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `AuditMaster_CheckExpectedActual` in `src/V12_002.REAPER.Audit.cs`
...
is internal to `AuditMaster_CheckExpectedActual` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `AuditMaster_CheckExpectedActual`
...
- **Source analyzed:** `docs/brain/EPIC-W7-083/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `AuditMaster_CheckExpectedActual` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `AuditMaster_CheckExpectedActual` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `AuditMaster_CheckExpectedActual` i
