# EPIC-W7-115 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-115/00-scope.md
...
**`SweepTrackedOrders` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SweepTrackedOrders` in `src/V12_002.SIMA.Lifecycle.cs`
...
is internal to `SweepTrackedOrders` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `SweepTrackedOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-115/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SweepTrackedOrders` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `SweepTrackedOrders` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `SweepTrackedOrders` in `src/V12_002.SIMA.Lifecy
