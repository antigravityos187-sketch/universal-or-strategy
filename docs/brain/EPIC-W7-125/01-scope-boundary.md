# EPIC-W7-125 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-125/00-scope.md
...
**`ShadowPropagateStopMoves` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ShadowPropagateStopMoves` in `src/V12_002.SIMA.Shadow.cs`
...
is internal to `ShadowPropagateStopMoves` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `ShadowPropagateStopMoves`
...
- **Source analyzed:** `docs/brain/EPIC-W7-125/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ShadowPropagateStopMoves` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `ShadowPropagateStopMoves` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ShadowPropagateStopMoves` in `src/V12_002
