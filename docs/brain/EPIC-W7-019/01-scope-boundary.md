# EPIC-W7-019 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-019/00-scope.md
...
**`TryHandleFleet_MoveTarget` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `TryHandleFleet_MoveTarget` in `src/V12_002.UI.IPC.Commands.Fleet.cs`
...
- `TryHandleFleetCommand` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `TryHandleFleet_MoveTarget` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `TryHandleFleet_MoveTarget`
...
- **Source analyzed:** `docs/brain/EPIC-W7-019/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `TryHandleFleet_MoveTarget` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** The **scope boundary** is strictly limited to the body of `TryHandleFleet_MoveTarget` (lines 645–693 in `src/V12_002.UI.IPC.Commands.Fleet.cs`) and any new private helper methods extracted from it wit
