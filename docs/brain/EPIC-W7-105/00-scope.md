...
> **Only `DrainAllDispatchQueuesOnAbort` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `DrainAllDispatchQueuesOnAbort` in `src/V12_002.SIMA.Fleet.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `DrainAllDispatchQueuesOnAbort` must remain
...
**Thought 1**: `DrainAllDispatchQueuesOnAbort` in `src/V12_002.SIMA.Fleet.cs` has CYC=3, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `DrainAllDispatchQueuesOnAbort` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `DrainAllDispatchQueuesOnAbort` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `DrainAllDispatchQueuesOnAbort`
...
| **Output** | docs/brain/EPIC-W7-105/00-scope.md |
