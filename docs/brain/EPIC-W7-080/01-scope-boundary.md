# EPIC-W7-080 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-080/00-scope.md
...
**`PlacePanel` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `PlacePanel` in `src/V12_002.UI.Panel.Construction.cs`
...
is internal to `PlacePanel` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `PlacePanel`
...
- **Source analyzed:** `docs/brain/EPIC-W7-080/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `PlacePanel` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `PlacePanel` and its new extracted helper methods are in scope.**
