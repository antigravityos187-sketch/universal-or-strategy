# B130 LaneA Deferred Backlog

**Block**: B130 LaneA — DW-B137: IsAtmSTPOrder Wrong Name Format
**Date**: 2026-09-01
**Status**: FINAL_PASS
...
|------|-------------|-----------|
| DW-B137 | `IsAtmSTPOrder` extended to cover Stop1/Stop2/Stop3 and Target1/Target2/Target3 ATM name formats. `SyncAtmFollowerTarget` added for ATM Limit target bracket cancel+resubmit. Branch (3b) added to `SyncFollowerBracket`. B129Tests.cs updated (3 Assert.False→Assert.True). B130Tests.cs created with 2 new [Fact] tests. | B130 LaneA T1 — BUILD_PASS, VERIFY_PASS |

---

## New Deferred Items — B130 LaneA

### DW-B137-SIM — Director SIM Gate: Stop1/Target1 Drag Sync Verification

**Priority**: P1
**Status**: OPEN
**Context**: Code fix is BUILD_PASS + VERIFY_PASS. Director must verify in live NT8 session:
- Enter a position using MES $200 SL 6 ATM template on leader (Sim101)
- Drag Stop1 on leader chart — confirm follower Stop1 updates via PTT-STP-Drag cancel+resubmit
- Drag Target1 on leader chart — confirm follower Target1 updates via PTT-TGT-Drag cancel+resubmit
- Confirm no naked positions after drag sync
**Deferred to**: Next Director SIM gate session after F5 compilation confirms no errors.

---

## Carry-Forward Items (from B129 LaneB-06-deferred-backlog.md — verbatim)

B130 LaneA changes (CopyEngine.cs 3 edits + B130Tests.cs new file + B129Tests.cs 3 assertion updates) do not affect any carry-forward item below except DW-B134-OCO (now covers both STP and TGT cancel+resubmit paths).

---

### DW-B134-OCO — OCO Orphan Risk After ATM Cancel+Resubmit
...
**B130 update**: Risk now applies to both `SyncAtmFollowerBracket` (stop) and `SyncAtmFollowerTarget`
(target): cancelling one leg of an ATM OCO pair may affect the partner leg. NT8 ATM OCO behavior on
`acc.Cancel` of a single bracket is unverified. Fix direction gated on Director SIM observation.
...
**Deferred to**: B131 or first SIM gate session after DW-B134 sync.
...
3. Enter a small test position on leader.
4. Click Quick2t — confirm Output tab shows `[PTT-QX-2T]` line with correct T1=/T2= values.
5. Confirm brackets submitted match qty split.
6. Enter a 1-contract position — click Quick2t — confirm T2 bracket NOT submitted.
7. Click QAll2t — confirm `[PTT-QX-ALL] GlobalQuickExit fired` appears in Output.
8. Confirm all accounts covered; no unprotected positions.
...
|----------|-------|-------|
| Closed this block (LaneA) | 1 | DW-B137 (Stop1/Target1 ATM name fix, BUILD_PASS, VERIFY_PASS) |
| New deferred (LaneA) | 1 | DW-B137-SIM (SIM gate verification) |
| Carry-forward (unchanged from B129 LaneB, except DW-B134-OCO updated) | 19 | DW-B134-OCO, DW-B129-01, DW-B133, DW-B124-01/02, DW-B107, B107-DEFER-01/02, DW-B42-01/02/03, DW-PTT-BE-FIX-01/02/03, DW-B89-DEFERRED-01/02/03/04/05/06 |

**Total open items**: 20 (1 new + 19 carry-forward)
