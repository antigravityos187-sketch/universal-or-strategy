# EPIC-W7-030 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-030/00-scope.md
...
**`ValidateOrphanedMasterOrders` + 4 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ValidateOrphanedMasterOrders` in `src/V12_002.Orders.Management.Cleanup.cs`
...
- `ReconcileOrphanedOrders` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `ValidateOrphanedMasterOrders` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `ValidateOrphanedMasterOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-030/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ValidateOrphanedMasterOrders` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 12, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Orders.Management.Cleanup.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** This phase enforces a strict **scope boundary**: exactly one **single method** is under
examination — `ValidateOrphanedMasterOrders`. No peer method, helper, caller, or
downstream dependency crosses i
