# EPIC-W7-018 — Phase 1.5: Scope Boundary Validation
...
**`IsSymbolMatch` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `IsSymbolMatch` in `src/V12_002.UI.IPC.cs`
...
- `ProcessIpcCommands` — confirmed not modified by this epic
- `IsCommandForThisInstrument` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `IsSymbolMatch` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `IsSymbolMatch`
...
- **Source analyzed:** `docs/brain/EPIC-W7-018/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `IsSymbolMatch` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.UI.IPC.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** This is a **single method** extraction epic. The **scope boundary** is defined as follows:

> Extract exactly the 15-term `isForMe` boolean expression at lines 321–337 of
> `IsCommandForThisInstrument
