# EPIC-W7-092 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-092/00-scope.md
...
**`SetRmaAnchorFromIpc` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SetRmaAnchorFromIpc` in `src/V12_002.SIMA.cs`
...
is internal to `SetRmaAnchorFromIpc` only — extracting sub-logic into private helpers without
...
- **Target file:** `src/V12_002.SIMA.cs`
...
are unaffected. The blast radius is contained to the single source file where `SetRmaAnchorFromIpc`
...
- **Source analyzed:** `docs/brain/EPIC-W7-092/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SetRmaAnchorFromIpc` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 2, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.SIMA.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `SetRmaAnchorFromIpc` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `SetRmaAnchorFromIpc` in `src/V12_002.SIMA.cs`.
