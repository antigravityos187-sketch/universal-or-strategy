# EPIC-W7-142 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-142/00-scope.md
...
**`HandleChartClick_ConvertPrice` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `HandleChartClick_ConvertPrice` in `src/V12_002.UI.Callbacks.cs`
...
is internal to `HandleChartClick_ConvertPrice` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `HandleChartClick_ConvertPrice`
...
- **Source analyzed:** `docs/brain/EPIC-W7-142/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `HandleChartClick_ConvertPrice` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.UI.Callbacks.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `HandleChartClick_ConvertPrice` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `HandleChartClick_ConvertPrice` in `s
