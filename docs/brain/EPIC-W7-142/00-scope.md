...
> **Only `HandleChartClick_ConvertPrice` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `HandleChartClick_ConvertPrice` in `src/V12_002.UI.Callbacks.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `HandleChartClick_ConvertPrice` must remain
...
**Thought 1**: `HandleChartClick_ConvertPrice` in `src/V12_002.UI.Callbacks.cs` has CYC=2, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `HandleChartClick_ConvertPrice` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `HandleChartClick_ConvertPrice` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `HandleChartClick_ConvertPrice`
- [x] Source file confirmed: `src/V12_002.UI.Callbacks.cs`
...
| **Output** | docs/brain/EPIC-W7-142/00-scope.md |
