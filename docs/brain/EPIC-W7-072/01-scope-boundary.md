# EPIC-W7-072 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-072/00-scope.md
...
**`ProcessAccountOrder_UpdateMasterExpected` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ProcessAccountOrder_UpdateMasterExpected` in `V12_002.Orders.Callbacks.AccountOrders.cs`
...
is internal to `ProcessAccountOrder_UpdateMasterExpected` only — extracting sub-logic into private helpers without
...
- **Target file:** `V12_002.Orders.Callbacks.AccountOrders.cs`
...
- **CYC baseline:** 12 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `ProcessAccountOrder_UpdateMasterExpected`
...
- **Source analyzed:** `docs/brain/EPIC-W7-072/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ProcessAccountOrder_UpdateMasterExpected` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `V12_002.Orders.Callbacks.AccountOrders.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `ProcessAccountOrder_UpdateMasterExpected` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ProcessAccountOrder_Updat
