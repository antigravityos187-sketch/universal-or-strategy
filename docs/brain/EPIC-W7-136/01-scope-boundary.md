# EPIC-W7-136 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-136/00-scope.md
...
**`ManageTrailingStops` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ManageTrailingStops` in `src/V12_002.Trailing.cs`
...
is internal to `ManageTrailingStops` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 327 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `ManageTrailingStops`
...
- **Source analyzed:** `docs/brain/EPIC-W7-136/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ManageTrailingStops` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `ManageTrailingStops` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ManageTrailingStops` in `src/V12_002.Trailing.
