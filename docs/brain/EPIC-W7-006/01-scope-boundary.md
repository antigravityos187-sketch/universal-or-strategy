# EPIC-W7-006 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-006/00-scope.md
...
**`AdoptFleetWorkingOrders` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `AdoptFleetWorkingOrders` in `src-vm-backup/V12_002.SIMA.Lifecycle.cs (backup location)`
...
is internal to `AdoptFleetWorkingOrders` only — extracting sub-logic into private helpers without
...
- **Target file:** `src-vm-backup/V12_002.SIMA.Lifecycle.cs (backup location)`
...
are unaffected. The blast radius is contained to the single source file where `AdoptFleetWorkingOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-006/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `AdoptFleetWorkingOrders` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 2, all confirmed upstream-only
- **Blast radius:** Limited to `src-vm-backup/V12_002.SIMA.Lifecycle.cs (backup location)` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `HydrateWorkingOrdersFromBroker` and its new extracted helper methods.**
