# B124 Deferred Backlog

Block: B124 (BE Button Active-State Brush + Arm Guard)
Date: 2026
...
|------|-------------|-----------|
| B124-REQ-1 | `_globalBeBtn2.Background = BrushActive` when BE-ALL armed (green) | B124-T1 Change A |
| B124-REQ-2 | `_globalBeBtn2.Background = Transparent` when idle (unchanged path confirmed) | B124-T1 Change A (idle branch untouched) |
| B124-REQ-3 | Double-press guard: logs `[PTT-BE-ALL] already armed, ignoring double-press` + `return` (no disarm, no re-arm) | B124-T1 Change B |
| B124-REQ-4 | xUnit Test 1 `GuardReturnsWithoutRearmingWhenAlreadyArmed` [Fact] present and confirmed | B124-T1 Test File |
| B124-REQ-5 | xUnit Test 2 `FirstPressArmsWhenNotYetArmed` [Fact] present and confirmed (with minor assertion caveat — see DW-B124-02 below) | B124-T1 Test File |
...
## Carry-Forward Items (from B107 — open items unchanged)

All items below are copied from `docs/brain/B107/06-deferred-backlog.md`.
B124 changes do not affect any of these items.
...
CS0433 Globals ambiguity at CopyEngine.cs:L3350. Confirmed pre-existing, unrelated to B124.
...
|----------|-------|-------|
| Closed this block | 5 | B124-REQ-1, B124-REQ-2, B124-REQ-3, B124-REQ-4, B124-REQ-5 |
| New deferred (B124 pipeline) | 2 | DW-B124-01 (behavioral change), DW-B124-02 (test assertion weakness) |
