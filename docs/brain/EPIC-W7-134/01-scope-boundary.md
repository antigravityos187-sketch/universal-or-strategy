# EPIC-W7-134 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-134/00-scope.md
...
**`MoveSpecificTarget` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `MoveSpecificTarget` in `src/V12_002.Trailing.Breakeven.cs`
...
Direct caller count (from 00-scope.md analysis): **18**
...
is internal to `MoveSpecificTarget` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `MoveSpecificTarget`
...
- **Source analyzed:** `docs/brain/EPIC-W7-134/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `MoveSpecificTarget` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 18, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Trailing.Breakeven.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `MoveSpecificTarget` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `MoveSpecificTarget` in `src/V12_002.Trailing.Br
