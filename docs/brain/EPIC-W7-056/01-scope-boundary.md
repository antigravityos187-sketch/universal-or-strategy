# EPIC-W7-056 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-056/00-scope.md
...
**`SweepBrokerOrders` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SweepBrokerOrders` in `src/V12_002.SIMA.Lifecycle.cs`
...
- `SweepTrackedOrders` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `SweepBrokerOrders` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 28 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `SweepBrokerOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-056/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SweepBrokerOrders` present and in-scope per Phase 1 analysis
...
- **IN scope:** The body of `SweepBrokerOrders` (lines 1360–1454 of
  `src/V12_002.SIMA.Lifecycle.cs`) and any new private helper methods extracted direc
