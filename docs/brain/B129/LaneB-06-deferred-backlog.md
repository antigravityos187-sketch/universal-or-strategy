...
|------|-------------|-----------|
| DW-B134 (partial) | ATM bracket drag sync — 3-layer fix (IsBracketLegStatic STP clause, IsAtmSTPOrder predicate, SyncFollowerBracket routing, SyncAtmFollowerBracket cancel+resubmit) | B129 LaneB T2 — BUILD_PASS, VERIFY_PASS |
| OQ-03 | Cascade safety question: cancel+resubmit of follower ATM bracket confirmed SAFE via Gate 2 (FindMatchingRule) null-return path at L1609. TryCancelFollowerEntries never reached for follower account orders. Confirmed by code inspection + passing xUnit test B129_DW134_OQ03. | B129 LaneB T2 |

---

## New Deferred Items — B129 LaneB
...
## Carry-Forward Items (from LaneA docs/brain/B129/06-deferred-backlog.md — verbatim)

B129 LaneB changes (CopyEngine.cs 4 edits + B129Tests.cs new file) do not affect any of the
items below. All are unchanged from the LaneA backlog.
...
|----------|-------|-------|
| Closed this block (LaneB) | 2 | DW-B134 (partial, 3-layer fix implemented), OQ-03 (answered + test verified) |
| New deferred (LaneB) | 1 | DW-B134-OCO (OCO orphan risk — P2, B130) |
| Carry-forward from LaneA (unchanged) | 18 | DW-B129-01, DW-B133, DW-B124-01/02, DW-B107, B107-DEFER-01/02, DW-B42-01/02/03, DW-PTT-BE-FIX-01/02/03, DW-B89-DEFERRED-01/02/03/04/05/06 |

**Total open items**: 19 (1 new + 18 carry-forward)

B129 LaneB changes do not affect any carry-forward item from LaneA.
