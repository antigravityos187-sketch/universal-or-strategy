...
> **Only `OnSubmitClick` and its new extracted helper methods are in scope.**
...
within the same partial class file. The public/internal signature of `OnSubmitClick` must remain
...
**Thought 1**: `OnSubmitClick` in `src/V12_002.UI.Panel.Handlers.cs` has CYC=34, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `OnSubmitClick` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `OnSubmitClick` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `OnSubmitClick`
...
| **Output** | docs/brain/EPIC-W7-075/00-scope.md |
