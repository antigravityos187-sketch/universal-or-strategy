...
| DW-B63-01 | B63/B74 | QX places targets but NO stop orders on followers. Root cause confirmed (B77 post-sign-off session): follower `snapshotStop = SnapshotStopPrice(follower, instr)` returns 0.0 because follower's ATM brackets haven't appeared in `acc.Orders` at QX fire time (NT8 ATM async lag). `pos.Quantity > 0` guard passes (follower IS filled), but `if (snapshotStop > 0)` guard at PttQuickExit.Execute:107 skips stop submission. Also: `followerTargets.Count = 0` so `targetCount = 2` fallback fires instead of using leader's real target count. Fix design: `ResolveStop(own, fallback)` + `ResolveTargetCount(own, leaderCount)` helpers in PttQuickExit; pass `leaderStop` + `leaderTargetCount` from PttGlobalQuickExit.Execute through ExecuteOne. CYC budget preserved (Execute drops 8→7). | P1 | OPEN -- root cause confirmed, fix design complete, awaiting Director approval for B78 direct engineer session |
