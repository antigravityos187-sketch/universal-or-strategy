...
**Thought 1**: `SymmetryGuardReplaceExistingFollowerTarget` in `src/V12_002.Symmetry.Replace.cs` has CYC=20, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `SymmetryGuardReplaceExistingFollowerTarget` only -- no scope creep.
...
| **Output** | docs/brain/EPIC-W7-128/00-scope.md |
