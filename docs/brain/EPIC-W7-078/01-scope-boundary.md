# EPIC-W7-078 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-078/00-scope.md
...
**`StopIpcServer` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `StopIpcServer` in `src/V12_002.UI.IPC.Server.cs`
...
is internal to `StopIpcServer` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `StopIpcServer`
...
- **Source analyzed:** `docs/brain/EPIC-W7-078/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `StopIpcServer` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `StopIpcServer` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `StopIpcServer` in `src/V12_002.UI.IPC.Server.cs`.
No
