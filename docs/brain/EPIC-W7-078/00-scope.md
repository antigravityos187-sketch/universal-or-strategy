...
> **Only `StopIpcServer` and its new extracted helper methods are in scope.**
...
within the same partial class file. The public/internal signature of `StopIpcServer` must remain
...
**Thought 1**: `StopIpcServer` in `src/V12_002.UI.IPC.Server.cs` has CYC=0, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `StopIpcServer` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `StopIpcServer` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `StopIpcServer`
...
| **Output** | docs/brain/EPIC-W7-078/00-scope.md |
