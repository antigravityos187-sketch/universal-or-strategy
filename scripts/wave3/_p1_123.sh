...
You are executing Phase 1 (Scope + Boundary) for EPIC-CCN-123.

**IMPORTANT**: Phase 1 now combines Scope Definition AND Boundary Validation (V12.25 10-phase workflow).

**Input Artifact**: Read `docs/brain/EPIC-CCN-123/00-hotspots.md` for hotspot analysis.

**Your Task**: Define the extraction scope AND validate boundary constraints.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-123/00-scope.md` with:
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_123.txt)" 2>&1 | tee logs/phase1/EPIC-CCN-123.log
echo "DONE_EXIT=$?"
