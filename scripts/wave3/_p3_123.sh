...
cat > /tmp/phase3_msg_123.txt << 'EOFMSG'
You are executing Phase 3 (DNA & PR Audit) for EPIC-CCN-123.

**Input Artifact**: Read `docs/brain/EPIC-CCN-123/02-implementation-plan.md` for architecture plan.

**Your Task**: Perform V12 DNA compliance checks and PR hygiene validation.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-123/03-audit-report.md` with:
...
bob --yolo --chat-mode advanced "$(cat /tmp/phase3_msg_123.txt)" 2>&1 | tee logs/phase3/EPIC-CCN-123.log
echo "DONE_EXIT=$?"
