...
cat > /tmp/phase3_msg_124.txt << 'EOFMSG'
You are executing Phase 3 (DNA & PR Audit) for EPIC-CCN-124.

**Input Artifact**: Read `docs/brain/EPIC-CCN-124/02-implementation-plan.md` for architecture plan.

**Your Task**: Perform V12 DNA compliance checks and PR hygiene validation.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-124/03-audit-report.md` with:
...
bob --yolo --chat-mode advanced "$(cat /tmp/phase3_msg_124.txt)" 2>&1 | tee logs/phase3/EPIC-CCN-124.log
echo "DONE_EXIT=$?"
