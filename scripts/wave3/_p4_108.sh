...
cat > /tmp/phase4_msg_108.txt << 'EOFMSG'
You are executing Phase 4 (Ticket Generation) for EPIC-CCN-108.

**Input Artifacts**: 
- Read `docs/brain/EPIC-CCN-108/02-architecture-plan.md` for extraction plan
- Read `docs/brain/EPIC-CCN-108/03-audit-report.md` for audit results

**Your Task**: Generate detailed implementation tickets for surgical extraction.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-108/04-tickets.md` with:
...
bob --yolo --chat-mode plan "$(cat /tmp/phase4_msg_108.txt)" 2>&1 | tee logs/phase4/EPIC-CCN-108.log
echo "DONE_EXIT=$?"
