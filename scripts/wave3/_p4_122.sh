...
cat > /tmp/phase4_msg_122.txt << 'EOFMSG'
You are executing Phase 4 (Ticket Generation) for EPIC-CCN-122.

**Input Artifacts**: 
- Read `docs/brain/EPIC-CCN-122/02-architecture-plan.md` for extraction plan
- Read `docs/brain/EPIC-CCN-122/03-audit-report.md` for audit results

**Your Task**: Generate detailed implementation tickets for surgical extraction.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-122/04-tickets.md` with:
...
2. Update `docs/brain/EPIC-CCN-122/manifest.json`:
...
bob --yolo --chat-mode plan "$(cat /tmp/phase4_msg_122.txt)" 2>&1 | tee logs/phase4/EPIC-CCN-122.log
echo "DONE_EXIT=$?"
