...
cat > /tmp/phase0_msg_125.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-125.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-125/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-125/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-125/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-125/manifest.json << 'EOF'
{
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-125/00-hotspots.md && wc -l docs/brain/EPIC-CCN-125/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-125/manifest.json && cat docs/brain/EPIC-CCN-125/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_125.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-125.log
echo "DONE_EXIT=$?"
