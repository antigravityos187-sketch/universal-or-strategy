...
if [ $? -ne 0 ]; then
    echo "❌ BLOCKED: V12.52 verification failed"
    exit 1
fi
echo "✅ Verification passed: Ready to execute (all gates passed)"
...
if [ $? -ne 0 ]; then
    echo "❌ Failed to start: $(python3 scripts/epic_manifest.py get_last_error "$EPIC_ID" "$PHASE")"
    exit 1
fi
PHASE_START_CLOCK=$(python3 -c "import json; m=json.load(open('docs/brain/$EPIC_ID/manifest.json')); print(m['phases']['$PHASE'].get('lamport_clock', 0))")
echo "✅ Phase started: Phase started (clock=$PHASE_START_CLOCK)"
...
PHASE_END_CLOCK=$(python3 -c "import json; m=json.load(open('docs/brain/$EPIC_ID/manifest.json')); print(m['phases']['$PHASE'].get('lamport_clock', 0))")
echo "✅ Phase completed: Phase completed (clock=$PHASE_END_CLOCK)"
