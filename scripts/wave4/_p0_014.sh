...
cat > /tmp/phase0_msg_014.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-014.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='TryHandleFleetCommand')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='TryHandleFleetCommand')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='TryHandleFleetCommand')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-014/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-014/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-014/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-014/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-014",
  "method": "TryHandleFleetCommand",
  "file": "src/V12_002.UI.IPC.Commands.Fleet.cs",
  "complexity": 19,
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-014/00-hotspots.md && wc -l docs/brain/EPIC-CCN-014/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-014/manifest.json && cat docs/brain/EPIC-CCN-014/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_014.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-014.log
echo "DONE_EXIT=$?"
