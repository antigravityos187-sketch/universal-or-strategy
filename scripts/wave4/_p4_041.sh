#!/bin/bash
# Phase 4 (Ticket Generation) for EPIC-CCN-041
...
cat > /tmp/phase4_msg_041.txt << 'EOFMSG'
Use the phase-4-tickets MCP server to execute Phase 4 for EPIC-CCN-041.

Call the execute_phase_4 tool with epic_id="EPIC-CCN-041".
...
bob --yolo "$(cat /tmp/phase4_msg_041.txt)"

# Verify file created
if [ -f "docs/brain/EPIC-CCN-041/04-tickets.md" ]; then
    echo "SUCCESS: Phase 4 complete for EPIC-CCN-041"
    echo "File: docs/brain/EPIC-CCN-041/04-tickets.md"
    ls -lh "docs/brain/EPIC-CCN-041/04-tickets.md"
else
