#!/bin/bash
# Phase 6 (Verification) for EPIC-CCN-069
set -e
...
if [ ! -f "docs/brain/EPIC-CCN-069/05-completion.md" ]; then
    echo "ERROR: Missing Phase 5 completion file for EPIC-CCN-069"
    exit 1
fi

# Create message file
cat > /tmp/phase6_msg_069.txt << 'EOFMSG'
Use the phase-6-review MCP server to execute Phase 6 for EPIC-CCN-069.

Call the execute_phase_6 tool with epic_id="EPIC-CCN-069".
...
bob --yolo "$(cat /tmp/phase6_msg_069.txt)"

# Verify completion report created (FIXED: correct filename)
if [ -f "docs/brain/EPIC-CCN-069/06-completion-report.md" ]; then
    echo "SUCCESS: Phase 6 complete for EPIC-CCN-069"
    echo "File: docs/brain/EPIC-CCN-069/06-completion-report.md"
    ls -lh docs/brain/EPIC-CCN-069/06-completion-report.md
else
    echo "ERROR: No completion report created for EPIC-CCN-069"
    exit 1
fi
