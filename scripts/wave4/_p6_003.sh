#!/bin/bash
# Phase 6 (Verification) for EPIC-CCN-003 - REGENERATED with correct prerequisite
set -e
...
if ! find docs/brain/EPIC-CCN-003 -maxdepth 1 \( -name "05-*.md" -o -name "ticket-*-completion.md" -o -name "ticket-completion.md" \) -print -quit | grep -q .; then
    ...
    echo "Expected: docs/brain/EPIC-CCN-003/05-*.md OR ticket-*-completion.md OR ticket-completion.md"
    exit 1
fi
...
if [ -f "docs/brain/EPIC-CCN-003/06-completion-report.md" ]; then
    ...
    echo "ERROR: No verification report created for EPIC-CCN-003"
    exit 1
fi
