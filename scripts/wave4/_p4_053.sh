#!/bin/bash
# Phase 4 (Ticket Generation) for EPIC-CCN-053
...
cat > /tmp/phase4_msg_053.txt << 'EOFMSG'
Use the phase-4-tickets MCP server to execute Phase 4 for EPIC-CCN-053.

Call the execute_phase_4 tool with epic_id="EPIC-CCN-053".
...
bob --yolo "$(cat /tmp/phase4_msg_053.txt)"

# Verify file created
if [ -f "docs/brain/EPIC-CCN-053/04-tickets.md" ]; then
    echo "SUCCESS: Phase 4 complete for EPIC-CCN-053"
    echo "File: docs/brain/EPIC-CCN-053/04-tickets.md"
    ls -lh "docs/brain/EPIC-CCN-053/04-tickets.md"
else
