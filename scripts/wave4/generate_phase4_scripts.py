#!/usr/bin/env python3
"""
Generate Phase 4 (Ticket Generation) scripts for Wave 4 using building-blocks method.

CRITICAL: This script uses the building-blocks method - it copies Phase 3 scripts
...
# Generate Phase 4 scripts (copy Phase 3 pattern)
...
for i, epic in enumerate(epics, 1):
    ...
    # Phase 4 script (uses phase-4-tickets MCP tool)
    # BUILDING-BLOCKS: Copied from Phase 3, changed only phase-specific parameters
...
cat > /tmp/phase4_msg_{epic_num}.txt << 'EOFMSG'
Use the phase-4-tickets MCP server to execute Phase 4 for {epic_id}.

Call the execute_phase_4 tool with epic_id="{epic_id}".
...
bob --yolo "$(cat /tmp/phase4_msg_{epic_num}.txt)"

# Verify file created
if [ -f "docs/brain/{epic_id}/04-tickets.md" ]; then
    echo "SUCCESS: Phase 4 complete for {epic_id}"
    echo "File: docs/brain/{epic_id}/04-tickets.md"
    ls -lh "docs/brain/{epic_id}/04-tickets.md"
else
    echo "ERROR: File not created for {epic_id}"
    exit 1
fi
'''
    
    # Write script
    script_path = output_dir / f"_p4_{epic_num}.sh"
...
# Phase 4 Test Launcher - First 2 Epics
# Generated: 2026-06-15

set -e

echo "[$(date)] Starting Phase 4 test launch (2 epics)"
...
screen -dmS p4-001 bash -l -c './scripts/wave4/_p4_001.sh 2>&1 | tee logs/phase4/EPIC-CCN-001.log'
...
screen -dmS p4-002 bash -l -c './scripts/wave4/_p4_002.sh 2>&1 | tee logs/phase4/EPIC-CCN-002.log'

echo "[$(date)] Test launch complete (2 epics)"
echo "Monitor with: screen -ls"
echo "Check files: ls docs/brain/EPIC-CCN-{001,002}/04-tickets.md"
'''

test_path = output_dir / "launch_phase4_test.sh"
...
full_path = output_dir / "launch_phase4_all.sh"
...
print(f"   - 80 epic scripts: _p4_001.sh through _p4_080.sh")
print(f"   - Test launcher: launch_phase4_test.sh")
print(f"   - Full launcher: launch_phase4_all.sh")
print(f"\nBuilding-blocks verification:")
print(f"   - Copied from Phase 3 pattern")
print(f"   - Changed: phase3 → phase4, Phase 3 → Phase 4, 03-audit-report.md → 04-tickets.md")
print(f"   - Preserved: cd command (line 7), API key export, bash -l -c pattern")
print(f"\nNext steps:")
print(f"   1. Review scripts with: diff scripts/wave4/_p3_001.sh scripts/wave4/_p4_001.sh")
print(f"   2. Upload to VM: gcloud compute scp scripts/wave4/_p4_*.sh scripts/wave4/launch_phase4_*.sh v12-test-golden-v2:~/universal-or-strategy/scripts/wave4/ --zone=us-central1-a")
print(f"   3. Run pilot test: gcloud compute ssh v12-test-golden-v2 --zone=us-central1-a --command='cd universal-or-strategy && ./scripts/wave4/launch_phase4_test.sh'")
