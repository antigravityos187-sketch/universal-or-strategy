...
mkdir -p docs/brain/EPIC-CCN-027
mkdir -p logs/phase0

cat > /tmp/phase0_msg_027.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-027.
...
2. Filter violations by file: src/V12_002.SIMA.Dispatch.cs
...
<execute_command>
<command>
grep -i "V12_002.SIMA.Dispatch.cs" jane_street_p0_violations.json | wc -l
</command>
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-027/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-027/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-027/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-027/manifest.json << 'EOF'
{
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-027/00-hotspots.md && wc -l docs/brain/EPIC-CCN-027/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-027/manifest.json && cat docs/brain/EPIC-CCN-027/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_027.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-027.log
echo "DONE_EXIT=$?"
