...
mkdir -p docs/brain/EPIC-001
mkdir -p logs/phase0

cat > /tmp/phase0_msg_001.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-001.
...
## Target Methods (6 methods in same file)
- Method 1: HandleSecondaryOrderFilled (CYC=21, LOC=69)
- Method 2: ProcessOnOrderUpdate (CYC=19, LOC=48)
- Method 3: RequestStopCancelLifecycleSafe (CYC=12, LOC=22)
- Method 4: HandleOrderRejected (CYC=12, LOC=29)
- Method 5: HandleOrderPriceOrQuantityChanged (CYC=11, LOC=37)
- Method 6: HandleOrderCancelled_ProcessStopReplacement (CYC=10, LOC=20)
- File: src/V12_002.Orders.Callbacks.cs
- Complexity: 21, 19, 12, 12, 11, 10 (target: all ≤8)
...
2. get_blast_radius(repo='universal-or-strategy', symbol='HandleSecondaryOrderFilled')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='HandleSecondaryOrderFilled')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='HandleSecondaryOrderFilled')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-001/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-001/00-hotspots.md << 'EOF'
# Phase 0: Hotspot Analysis - EPIC-001
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-001/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-001/manifest.json << 'EOF'
{
  "epic_id": "EPIC-001",
  "methods": ["HandleSecondaryOrderFilled", "ProcessOnOrderUpdate", "RequestStopCancelLifecycleSafe", "HandleOrderRejected", "HandleOrderPriceOrQuantityChanged", "HandleOrderCancelled_ProcessStopReplacement"],
  "file": "src/V12_002.Orders.Callbacks.cs",
  "complexity": [21, 19, 12, 12, 11, 10],
  "phases": {
    "0": {
      "status": "completed",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-001/00-hotspots.md && wc -l docs/brain/EPIC-001/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-001/manifest.json && cat docs/brain/EPIC-001/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_001.txt)" 2>&1 | tee logs/phase0/EPIC-001.log
echo "DONE_EXIT=$?"
