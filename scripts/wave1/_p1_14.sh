...
mkdir -p docs/brain/EPIC-014
mkdir -p logs/phase1

cat > /tmp/phase1_msg_003.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition) for EPIC-014.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-014/00-scope.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-014/00-scope.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-014/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-014/manifest.json << 'EOF'
{
  "epic_id": "EPIC-014",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-014/00-scope.md && wc -l docs/brain/EPIC-014/00-scope.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-014/manifest.json && cat docs/brain/EPIC-014/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_003.txt)" 2>&1 | tee logs/phase1/EPIC-014.log
echo "DONE_EXIT=$?"
