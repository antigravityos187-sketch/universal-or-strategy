...
mkdir -p docs/brain/EPIC-005
mkdir -p logs/phase0

cat > /tmp/phase0_msg_005.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-005.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='EmergencyFlattenSingleFleetAccount')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='EmergencyFlattenSingleFleetAccount')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='EmergencyFlattenSingleFleetAccount')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-005/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-005/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-005/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-005/manifest.json << 'EOF'
{
  "epic_id": "EPIC-005",
  "methods": ["EmergencyFlattenSingleFleetAccount", "FlattenFleetPosition"],
  "file": "src/V12_002.SIMA.Flatten.cs",
  "complexity": [16, 9],
  "phases": {
    "0": {
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-005/00-hotspots.md && wc -l docs/brain/EPIC-005/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-005/manifest.json && cat docs/brain/EPIC-005/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_005.txt)" 2>&1 | tee logs/phase0/EPIC-005.log
echo "DONE_EXIT=$?"
