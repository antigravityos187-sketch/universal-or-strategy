#!/bin/bash
# Phase 5 (Ticket Execution) for EPIC-CCN-075
...
if [ ! -f "docs/brain/EPIC-CCN-075/04-tickets.md" ]; then
    echo "ERROR: Missing prerequisite file: docs/brain/EPIC-CCN-075/04-tickets.md"
    echo "Phase 4 must complete before Phase 5 can execute"
    exit 1
fi

# Create message file (avoids bash multi-line escaping)
cat > /tmp/phase5_msg_075.txt << 'EOFMSG'
Use the phase-5-execute MCP server to execute Phase 5 for EPIC-CCN-075.

Call the execute_phase_5 tool with epic_id="EPIC-CCN-075".
...
bob --yolo --chat-mode v12-engineer "$(cat /tmp/phase5_msg_075.txt)"

# Verify execution files created (at least one ticket completion file)
if ls docs/brain/EPIC-CCN-075/ticket-*-completion.md 1> /dev/null 2>&1; then
    echo "SUCCESS: Phase 5 complete for EPIC-CCN-075"
    echo "Files: docs/brain/EPIC-CCN-075/ticket-*-completion.md"
    ls -lh docs/brain/EPIC-CCN-075/ticket-*-completion.md
else
    echo "ERROR: No ticket completion files created for EPIC-CCN-075"
    exit 1
fi
