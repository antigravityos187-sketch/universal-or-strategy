#!/usr/bin/env python3
"""
Phase 4 Launch with Self-Healing Checkpoint System (v3 - FIXED)

Fixes from v2:
1. Uses PROVEN API allocation from Wave 2 v4 (lines 57-65 of _wave2_v4_launch_generated.sh)
2. Validates for duplicate keys before launch
3. References source script in comments

CRITICAL: This allocation is IMMUTABLE - copied from successful Wave 2 v4 run.
"""
...
# API allocation - PROVEN from Wave 2 v4 (scripts/wave2/_wave2_v4_launch_generated.sh lines 57-65)
# DO NOT MODIFY - This allocation is IMMUTABLE
API_ALLOCATION = {
    "107": "b (2).json",
    "108": "b.json",
    "109": "bob (1).json",
    ...
    "112": "bob (4).json",
    "113": "bob (5).json",
    "114": "bob (6).json",  # CRITICAL: NOT b.json (would duplicate 108)
    "115": "bob.json",
}
...
def validate_api_allocation():
    ...
    print(f"[VALIDATE] OK - {len(api_values)} unique API keys validated")
...
def build_phase4_script(epics_to_run: list) -> str:
    ...
    script_lines = [
        "#!/bin/bash",
        "# Phase 4: Ticket Generation with Validated API Allocation (v3)",
        "# API Allocation: PROVEN from Wave 2 v4 (scripts/wave2/_wave2_v4_launch_generated.sh)",
        "set -e",
        "",
    ...
    for epic in epics_to_run:
        ...
        script_lines.extend([
            f"# EPIC-CCN-{epic_id} -> {api_file}",
            f"echo '[PHASE4] Starting EPIC-CCN-{epic_id}...'",
            f"screen -dmS phase4-EPIC-{epic_id} bash -l -c \"",
...
def launch_agents_on_vm(epics_to_run: list) -> bool:
    ...
    script_path = Path("/tmp/wave2_phase4_v3_fixed.sh")
    ...
    try:
        ...
        subprocess.run([
            ...
            str(script_path),
            f"{VM_NAME}:/tmp/wave2_phase4_v3_fixed.sh",
            f"--zone={ZONE}"
        ...
        result = subprocess.run([
            ...
            f"--zone={ZONE}",
            "--command=bash /tmp/wave2_phase4_v3_fixed.sh"
        ], check=True, capture_output=True, text=True, shell=True)
...
def main():
    print("[PHASE4] Starting Phase 4 with Validated API Allocation (v3 - FIXED)")
    print("=" * 60)
    
    # MANDATORY: Validate API allocation before ANY launch
    try:
        validate_api_allocation()
    except ValueError as e:
        print(f"\n[FATAL] {e}")
        print("[ABORT] Fix API_ALLOCATION before launching")
        return
    
    # Check which epics need Phase 4
    epics_to_run = []
    epics_completed = []
    epics_in_progress = []
    ...
    # ONLY NOW mark as in_progress (after successful launch)
    print("\n[MARK] Marking epics as in_progress...")
    for epic in epics_to_run:
        update_manifest(epic["id"], PHASE, "in_progress")
    
    print("\n[SUCCESS] Phase 4 launched with VALIDATED allocation!")
...
# Made with Bob - Validated Allocation Edition