...
{
    public class UniversalORStrategyV12_FULL_UI : Strategy
    {
        #region Variables
        ...
        private TextBlock complianceSummaryText; // V12.11: Apex compliance + performance summary
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                Name = "UniversalORStrategyV12_FULL_UI";
        ...
        private System.Windows.Controls.Grid CreateDoubleHorizonDashboard()
        {
            ...
            // V12.11: Apex Compliance + Performance Metrics
            StackPanel complianceStack = new StackPanel { Orientation = Orientation.Vertical, Margin = new Thickness(20, 0, 0, 0), VerticalAlignment = VerticalAlignment.Center };
            ...
            complianceStack.Children.Add(complianceSummaryText);
            complianceStack.Children.Add(complianceConsistencyText);
            complianceStack.Children.Add(compliancePayoutText);
            complianceStack.Children.Add(complianceDrawdownText);
            dataRibbon.Children.Add(complianceStack);
        ...
        private void UpdateDisplay()
        {
            ...
            // Build compliance snapshot on strategy thread
            ComplianceSnapshot complianceSnapshot = BuildComplianceSnapshot();

            // Pass the SNAPSHOT to the UI thread
            ChartControl.Dispatcher.InvokeAsync(() => UpdateDisplayInternal(snapshot, complianceSnapshot));
        }

        private void UpdateDisplayInternal(List<PositionDisplayInfo> positions, ComplianceSnapshot complianceSnapshot)
        {
            if (!uiCreated) return;
            ...
            try
            {
                ...
                // V12.11: Apex Compliance UI updates
                if (complianceSummaryText != null)
                {
                    if (!complianceSnapshot.Enabled)
                    {
                        complianceSummaryText.Text = "COMPLIANCE: OFF";
                        complianceSummaryText.Foreground = DimText;
                        if (complianceConsistencyText != null)
                        {
                            complianceConsistencyText.Text = "CONSISTENCY: --";
                            complianceConsistencyText.Foreground = DimText;
                        }
                        if (compliancePayoutText != null)
                        {
                            compliancePayoutText.Text = "PAYOUT: --";
                            compliancePayoutText.Foreground = DimText;
                        }
                        if (complianceDrawdownText != null)
                        {
                            complianceDrawdownText.Text = "DD BUFFER: --";
                            complianceDrawdownText.Foreground = DimText;
                        }
                    }
                    else if (!complianceSnapshot.HasAccounts)
                    {
                        complianceSummaryText.Text = "ACCT: -- | TRADES: 0 | DAYS: 0 | MAXDD: $0";
                        complianceSummaryText.Foreground = DimText;
                        if (complianceConsistencyText != null)
                        {
                            complianceConsistencyText.Text = "CONSISTENCY: --";
                    ...
                    {
                        complianceSummaryText.Text = string.Format("ACCT: {0} | TRADES: {1} | DAYS: {2} | MAXDD: ${3:F0}",
                            complianceSnapshot.AccountName, complianceSnapshot.TradeCount, complianceSnapshot.UniqueDays, complianceSnapshot.MaxDrawdown);
                        complianceSummaryText.Foreground = MutedText;

                        if (complianceConsistencyText != null)
                        {
                            complianceConsistencyText.Text = complianceSnapshot.ConsistencyText;
                            complianceConsistencyText.Foreground = GetComplianceSeverityBrush(complianceSnapshot.ConsistencySeverity);
                        }
                        if (compliancePayoutText != null)
                        {
                            compliancePayoutText.Text = complianceSnapshot.PayoutText;
                            compliancePayoutText.Foreground = GetComplianceSeverityBrush(complianceSnapshot.PayoutSeverity);
                        }
                        if (complianceDrawdownText != null)
                        {
                            complianceDrawdownText.Text = complianceSnapshot.DrawdownText;
                            complianceDrawdownText.Foreground = GetComplianceSeverityBrush(complianceSnapshot.DrawdownSeverity);
                        }
                    }
                }
        ...
        private void SendResponseToRemote(string response)
{
    ...
    foreach (var client in connectedClients)
    {
        ...
        private Brush GetComplianceSeverityBrush(int severity)
        {
            if (severity >= 2) return OffRed;
            if (severity == 1) return OrangeForeground;
            return SignalBullishFg;
        }
