...
# Set API key
export BOB_API_KEY_FILE="$HOME/.bob/api-keys/bob (4).json"
...
bob --mode advanced \
    --message "Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-112.

**CRITICAL**: You MUST write files to disk and verify they exist.

## Target Method
- Method: ExtractTargetConfiguration
- File: src/V12_002.cs
- Complexity: 31
...
2. get_blast_radius(repo='universal-or-strategy', symbol='ExtractTargetConfiguration')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='ExtractTargetConfiguration')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='ExtractTargetConfiguration')

### Step 2: Write 00-hotspots.md
Use write_to_file tool to create docs/brain/EPIC-CCN-112/00-hotspots.md with:
...
Use write_to_file tool to create docs/brain/EPIC-CCN-112/manifest.json:
```json
{
...
1. read_file docs/brain/EPIC-CCN-112/00-hotspots.md
2. read_file docs/brain/EPIC-CCN-112/manifest.json
