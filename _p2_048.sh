...
export BOBSHELL_API_KEY='bob_prod_bob-admin_6BiuAbjqXBKMoPe4kBFbSmDCV1p3zprcVrubyFtju118kiJ29n296EKiSxtcM8SBWk6X5RTpnYQdkKxdSHnAjWh_HXNWFe61d6cKaPstZ3qYTWXscuQ2TrfDXa1UWjhT1YKv'
mkdir -p docs/brain/EPIC-W7-048
mkdir -p logs/wave7/phase2

if [ ! -f "docs/brain/EPIC-W7-048/01-scope-boundary.md" ]; then
    echo "BLOCKED: Phase 1.5 not complete"
    exit 1
fi

cat > /tmp/phase2_msg_048.txt << 'EOFMSG'
Execute Phase 2 (Architecture Planning) for EPIC-W7-048.
...
Input: docs/brain/EPIC-W7-048/01-scope-boundary.md
...
4. Write docs/brain/EPIC-W7-048/02-architecture-plan.md using execute_command
...
bob --yolo --chat-mode plan "$(cat /tmp/phase2_msg_048.txt)" 2>&1 | tee logs/wave7/phase2/EPIC-W7-048.log
echo "DONE_EXIT=$?"
