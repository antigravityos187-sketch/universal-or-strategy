    ...
    internal static class LicenseClient
    {
        ...
        private static CacheEntry DeserializeCache(string json)
        {
            try
            {
                ...
                {
                    Key        = key,
                    ExpiresUtc = System.DateTime.Parse(exp,
        ...
        private static string ExtractJsonString(string json, string field)
        {
            ...
            if (start < 0) return null;   // branch 1
            start += marker.Length;
            ...
            string json, string field)
        {
            ...
            var marker = "\"" + field + "\":[";
            int start  = json.IndexOf(marker, System.StringComparison.Ordinal);
            if (start < 0) return result;   // branch 1
            start += marker.Length;
            int end = json.IndexOf(']', start);
            if (end < 0) return result;     // branch 2
            var inner = json.Substring(start, end - start).Trim();
            if (inner.Length == 0) return result;  // branch 3
        ...
        private static void WriteCache(string key, FeatureFlags flags)
        {
            try
            {
                ...
                sb.Append("{");
                sb.Append("\"key\":\"");         sb.Append(EscapeJson(key));          sb.Append("\",");
                sb.Append("\"cached_utc\":\"");  sb.Append(System.DateTime.UtcNow.ToString("o")); sb.Append("\",");
                sb.Append("\"expires_utc\":\""); sb.Append(System.DateTime.UtcNow.AddDays(CacheDaysValid).ToString("o")); sb.Append("\",");
                sb.Append("\"features\":[");
                for (int i = 0; i < feats.Count; i++)  // branch 1 (loop)
                {
                    if (i > 0) sb.Append(",");           // branch 2
                    sb.Append("\""); sb.Append(EscapeJson(feats[i])); sb.Append("\"");
                }
                sb.Append("]}");
        ...
        private static System.Collections.Generic.List<string> GetFeatureList(FeatureFlags f)
        {
            ...
            if (f.MultiRule)    list.Add("multi_rule");
            if (f.TrimFlatten)  list.Add("trim_flatten");
            if (f.BreakEven)    list.Add("break_even");
            if (f.AtrSizing)    list.Add("atr_sizing");
            if (f.ClickTrader)  list.Add("click_trader");
            if (f.MirrorMode)   list.Add("mirror_mode");
            if (f.QxGlobalExit) list.Add("qx_global_exit");
            return list;
        }
