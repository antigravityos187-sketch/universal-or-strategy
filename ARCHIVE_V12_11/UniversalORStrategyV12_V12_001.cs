...
{
    public class UniversalORStrategyV12_V12_001 : Strategy
    {
        #region Variables
        ...
        private class PositionInfo
        {
            ...
            public bool T4Filled;       // v5.13: New flag
        ...
        protected override void OnStateChange()
        {
            if (State == State.SetDefaults)
            {
                ...
                Name = "UniversalORStrategyV12_V12_001";
        ...
        private void ExecuteTargetAction(string targetType, string action)
        {
            try
            {
                ...
                foreach (var kvp in activePositions.ToArray())
                {
                    ...
                    ConcurrentDictionary<string, Order> targetOrders = (targetType == "T1") ? target1Orders : (targetType == "T2" ? target2Orders : (targetType == "T3" ? target3Orders : target4Orders));
                    int targetContracts = (targetType == "T1") ? pos.T1Contracts : (targetType == "T2" ? pos.T2Contracts : (targetType == "T3" ? pos.T3Contracts : pos.T4Contracts));
                    bool targetFilled = (targetType == "T1") ? pos.T1Filled : (targetType == "T2" ? pos.T2Filled : (targetType == "T3" ? pos.T3Filled : pos.T4Filled));
        ...
        private void SendResponseToRemote(string response)
{
    ...
    foreach (var client in connectedClients)
    {
        ...
        private void BroadcastIPC(string message)
        {
            if (connectedClients == null) return;
            ...
            foreach (var client in connectedClients)
            {
                ...
                {
                    // Client likely disconnected; ignore to keep broadcasts stable
                }
            }
        }
        ...
        private void BroadcastComplianceSummary()
        {
            ComplianceSnapshot snapshot = BuildComplianceSnapshot();

            string summary = string.Format("ACCT: {0} / TRADES: {1} / DAYS: {2} / MAXDD: ${3:F0}",
                snapshot.AccountName, snapshot.TradeCount, snapshot.UniqueDays, snapshot.MaxDrawdown);

            string message = string.Format("COMPLIANCE|{0}|{1}|{2}|{3}",
                summary, snapshot.ConsistencyText, snapshot.PayoutText, snapshot.DrawdownText);

            BroadcastIPC(message);
        }
