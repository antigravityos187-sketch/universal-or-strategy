...
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-113.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-113/00-hotspots.md:

```bash
cat > docs/brain/EPIC-CCN-113/00-hotspots.md << 'EOF'
...
Use execute_command to create docs/brain/EPIC-CCN-113/manifest.json:

```bash
cat > docs/brain/EPIC-CCN-113/manifest.json << 'EOF'
{
...
```bash
ls -lh docs/brain/EPIC-CCN-113/00-hotspots.md && wc -l docs/brain/EPIC-CCN-113/00-hotspots.md
```

2. Verify manifest.json:
```bash
ls -lh docs/brain/EPIC-CCN-113/manifest.json && cat docs/brain/EPIC-CCN-113/manifest.json | head -20
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_113.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-113.log
echo "DONE_EXIT=$?"
