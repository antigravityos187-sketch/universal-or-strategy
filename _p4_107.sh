...
cat > /tmp/phase4_msg_107.txt << 'EOFMSG'
You are executing Phase 4 (Ticket Generation) for EPIC-CCN-107.

**Input Artifacts**: 
- Read `docs/brain/EPIC-CCN-107/02-architecture-plan.md` for extraction plan
- Read `docs/brain/EPIC-CCN-107/03-audit-report.md` for audit results

**Your Task**: Generate detailed implementation tickets for surgical extraction.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-107/04-tickets.md` with:
...
bob --yolo --chat-mode plan "$(cat /tmp/phase4_msg_107.txt)" 2>&1 | tee logs/phase4/EPIC-CCN-107.log
echo "DONE_EXIT=$?"
