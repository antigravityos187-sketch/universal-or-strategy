...
cat > /tmp/phase0_msg_111.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-111.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='HydrateExpectedPositionsFromBroker')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='HydrateExpectedPositionsFromBroker')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='HydrateExpectedPositionsFromBroker')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-111/00-hotspots.md:

```bash
cat > docs/brain/EPIC-CCN-111/00-hotspots.md << 'EOF'
...
Use execute_command to create docs/brain/EPIC-CCN-111/manifest.json:

```bash
cat > docs/brain/EPIC-CCN-111/manifest.json << 'EOF'
{
...
```bash
ls -lh docs/brain/EPIC-CCN-111/00-hotspots.md && wc -l docs/brain/EPIC-CCN-111/00-hotspots.md
```

2. Verify manifest.json:
```bash
ls -lh docs/brain/EPIC-CCN-111/manifest.json && cat docs/brain/EPIC-CCN-111/manifest.json | head -20
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_111.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-111.log
echo "DONE_EXIT=$?"
