...
cat > /tmp/phase3_msg_108.txt << 'EOFMSG'
You are executing Phase 3 (DNA & PR Audit) for EPIC-CCN-108.

**Input Artifact**: Read `docs/brain/EPIC-CCN-108/02-architecture-plan.md` for architecture plan.

**Your Task**: Perform V12 DNA compliance checks and PR hygiene validation.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-108/03-audit-report.md` with:
...
bob --yolo --chat-mode advanced "$(cat /tmp/phase3_msg_108.txt)" 2>&1 | tee logs/phase3/EPIC-CCN-108.log
echo "DONE_EXIT=$?"
