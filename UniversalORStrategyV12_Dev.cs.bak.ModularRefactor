...
{
    public class UniversalORStrategyV12_Dev : Strategy
    {
        #region Variables
        ...
        protected override void OnStateChange()
        {
            ...
            {
                Description = "Universal OR Strategy V12 - Dev Build";
                Name = "UniversalORStrategyV12_Dev";
            ...
            else if (State == State.DataLoaded)
            {
                ...
                Print(string.Format("UniversalORStrategy V12.12 | {0} | Tick: {1} | PV: ${2}", symbol, tickSize, pointValue));
            ...
            DateTime time, ErrorCode error, string nativeError)
        {
            try
            {
                ...
                // V12.13: Coordination flag — prevents redundant ghost-scan after explicit handling
                ...
                // V12.13c: Manual stop cancel — clean up dictionary ref, no auto re-submission
                    ...
                    (orderName.StartsWith("Stop_") || orderName.StartsWith("S_")))
                {
                    foreach (var kvp in stopOrders.ToArray())
                    {
                        if (kvp.Value == order)
                        {
                            ...
                            {
                                Print(string.Format("V12.13c: Stop cancelled: {0} — removed from tracking", entryName));
                                handledByExplicitCleanup = true;
                            }
                            break;
                        }
                ...
                // V12.13: Manual target cancel — user cancelled target from chart
                if (!handledByExplicitCleanup && orderState == OrderState.Cancelled)
                {
                    ...
                    foreach (var (dict, label) in targetDicts)
                    {
                        foreach (var kvp in dict.ToArray())
                        {
                            if (kvp.Value == order)
                            {
                                ...
                                {
                                    Print(string.Format("V12.13: {0} MANUALLY CANCELLED: {1} — contracts run with stop", label, kvp.Key));
                                    handledByExplicitCleanup = true;
                                }
                                break;
                            }
                ...
                if (entryOrders.Values.Contains(order) && orderState == OrderState.Cancelled)
                {
                    ...
                    foreach (var kvp in activePositions.ToArray())
                    {
                        ...
                        if (entryOrders.TryGetValue(entryName, out var entryOrder) && entryOrder == order && !pos.EntryFilled)
                        {
                            Print(string.Format("V12.13: Entry CANCELLED — full teardown: {0}", entryName));
                ...
                // V12.13: Terminal catch-all — ONLY fires if no explicit handler above already cleaned this order
        ...
        private void RemoveGhostOrderRef(Order order, string reason)
        {
            ...
            {
                // V8.30: Atomic check and removal
                foreach (var kvp in dict.ToArray())
                {
                    if (kvp.Value == order)
                    {
                        if (dict.TryRemove(kvp.Key, out _))
                        {
                            Print(string.Format("V12.12b: {0} {1} - removed internal ghost ref for {2}", label, reason, kvp.Key));
                            foundInDict = true;
                        }
                    }
                }
            ...
            // V12.13: If it wasn't in our dictionaries, classify why
            if (!foundInDict)
            {
                // Only log if it's one of our orders (matching prefix) to avoid noise from other strategies
                if (order.Name.Contains("RMA") || order.Name.Contains("OR") || order.Name.Contains("MOMO") || order.Name.Contains("TREND"))
                {
                    // V12.13: Distinguish expected cascade from suspicious orphan
                    ...
                    foreach (var kvp in activePositions.ToArray())
                    {
                        if (order.Name.Contains(kvp.Key))
                        {
                            ...
                            Print(string.Format("V12.13: WARNING {0} {1} — dict ref gone but position {2} still active (orphan risk)",
                                order.Name, reason, kvp.Key));
                            break;
                        }
                    }
                    if (!positionStillActive)
                    {
                        Print(string.Format("V12.13: {0} {1} — cleaned by upstream handler (expected cascade)", order.Name, reason));
                    }
                }
            }
        }
