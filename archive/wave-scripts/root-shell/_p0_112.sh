...
Execute Phase 0 (Hotspot Analysis) for EPIC-W7-112.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-W7-112/00-hotspots.md:

```bash
cat > docs/brain/EPIC-W7-112/00-hotspots.md << 'EOF'
...
Use execute_command to create docs/brain/EPIC-W7-112/manifest.json:

```bash
cat > docs/brain/EPIC-W7-112/manifest.json << 'EOF'
{
  "epic_id": "EPIC-W7-112",
  "method": "ClassifyOrderByPrefix",
  "file": "src/V12_002.SIMA.Lifecycle.cs",
...
```bash
ls -lh docs/brain/EPIC-W7-112/00-hotspots.md && wc -l docs/brain/EPIC-W7-112/00-hotspots.md
```

2. Verify manifest.json:
```bash
ls -lh docs/brain/EPIC-W7-112/manifest.json && cat docs/brain/EPIC-W7-112/manifest.json | head -20
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_112.txt)" 2>&1 | tee logs/phase0/EPIC-W7-112.log
echo "DONE_EXIT=$?"
