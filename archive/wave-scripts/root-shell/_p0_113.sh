...
mkdir -p docs/brain/EPIC-W7-113
mkdir -p logs/phase0

cat > /tmp/phase0_msg_113.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-W7-113.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-W7-113/00-hotspots.md:

```bash
cat > docs/brain/EPIC-W7-113/00-hotspots.md << 'EOF'
...
Use execute_command to create docs/brain/EPIC-W7-113/manifest.json:

```bash
cat > docs/brain/EPIC-W7-113/manifest.json << 'EOF'
{
  "epic_id": "EPIC-W7-113",
  "method": "HydrateFSMsFromWorkingOrders",
  "file": "src/V12_002.SIMA.Lifecycle.cs",
...
```bash
ls -lh docs/brain/EPIC-W7-113/00-hotspots.md && wc -l docs/brain/EPIC-W7-113/00-hotspots.md
```

2. Verify manifest.json:
```bash
ls -lh docs/brain/EPIC-W7-113/manifest.json && cat docs/brain/EPIC-W7-113/manifest.json | head -20
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_113.txt)" 2>&1 | tee logs/phase0/EPIC-W7-113.log
echo "DONE_EXIT=$?"
