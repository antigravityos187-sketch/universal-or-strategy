...
mkdir -p docs/brain/EPIC-W7-023
mkdir -p logs/phase0

cat > /tmp/phase0_msg_023.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-W7-023.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-W7-023/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-W7-023/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-W7-023/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-W7-023/manifest.json << 'EOF'
{
  "epic_id": "EPIC-W7-023",
  "method": "HandleFlatPosition_CleanupActivePositions",
  "file": "src/V12_002.Orders.Callbacks.Execution.cs",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-W7-023/00-hotspots.md && wc -l docs/brain/EPIC-W7-023/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-W7-023/manifest.json && cat docs/brain/EPIC-W7-023/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_023.txt)" 2>&1 | tee logs/phase0/EPIC-W7-023.log
echo "DONE_EXIT=$?"
