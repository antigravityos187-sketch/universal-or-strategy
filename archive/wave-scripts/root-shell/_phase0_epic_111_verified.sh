...
# Set API key
export BOB_API_KEY_FILE="$HOME/.bob/api-keys/bob (3).json"
...
bob --mode advanced \
    --message "Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-111.

**CRITICAL**: You MUST write files to disk and verify they exist.

## Target Method
- Method: AdoptFleetOrders
- File: src/V12_002.cs
- Complexity: 37
...
2. get_blast_radius(repo='universal-or-strategy', symbol='AdoptFleetOrders')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='AdoptFleetOrders')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='AdoptFleetOrders')

### Step 2: Write 00-hotspots.md
Use write_to_file tool to create docs/brain/EPIC-CCN-111/00-hotspots.md with:
...
Use write_to_file tool to create docs/brain/EPIC-CCN-111/manifest.json:
```json
{
...
1. read_file docs/brain/EPIC-CCN-111/00-hotspots.md
2. read_file docs/brain/EPIC-CCN-111/manifest.json
