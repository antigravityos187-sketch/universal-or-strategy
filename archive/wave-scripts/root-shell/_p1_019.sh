...
mkdir -p docs/brain/EPIC-W7-019
mkdir -p logs/wave7/phase1

if [ ! -f "docs/brain/EPIC-W7-019/00-hotspots.md" ]; then
    echo "BLOCKED: Phase 0 not complete"
    exit 1
fi

cat > /tmp/phase1_msg_019.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition) for EPIC-W7-019.
...
Input: docs/brain/EPIC-W7-019/00-hotspots.md

Required Actions:
1. Read hotspot analysis
2. Define extraction scope (IN SCOPE vs OUT OF SCOPE)
3. Write docs/brain/EPIC-W7-019/00-scope.md using execute_command
...
~/.npm-global/bin/bob --yolo --chat-mode v12-phase1-scope "$(cat /tmp/phase1_msg_019.txt)"
DONE_EXIT=$?
echo "Phase 1 complete for EPIC-W7-019 (exit: $DONE_EXIT)"
exit $DONE_EXIT
