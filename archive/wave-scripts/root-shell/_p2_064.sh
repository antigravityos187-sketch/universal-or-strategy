...
if [ ! -f "docs/brain/EPIC-W7-064/01-scope-boundary.md" ]; then
    echo "BLOCKED: Phase 1.5 not complete"
    exit 1
fi

cat > /tmp/phase2_msg_064.txt << 'EOFMSG'
Execute Phase 2 (Architecture Planning) for EPIC-W7-064.
...
4. Write docs/brain/EPIC-W7-064/02-architecture-plan.md using execute_command
...
bob --yolo --chat-mode plan "$(cat /tmp/phase2_msg_064.txt)" 2>&1 | tee logs/wave7/phase2/EPIC-W7-064.log
echo "DONE_EXIT=$?"
