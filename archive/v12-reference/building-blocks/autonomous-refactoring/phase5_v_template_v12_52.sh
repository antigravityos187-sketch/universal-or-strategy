...
bob --mode advanced --task "Verify ticket $TICKET_ID execution for $EPIC_ID based on completion file $COMPLETION_FILE. Check: (1) compilation passes, (2) CYC ≤8 achieved, (3) only target method modified, (4) xUnit tests passing, (5) UTF-8 encoding. Use jCodemunch for complexity verification. Output: $OUTPUT_FILE" \
    --context "$COMPLETION_FILE" \
    --output "$OUTPUT_FILE" \
    2>&1 | tee "logs/wave6/phase5_v/$EPIC_ID-ticket-$TICKET_ID.log"
