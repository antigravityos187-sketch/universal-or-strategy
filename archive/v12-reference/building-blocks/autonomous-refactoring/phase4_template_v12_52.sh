...
bob --mode plan --task "Generate surgical extraction tickets for $EPIC_ID based on audit report in $AUDIT_FILE. Use jCodemunch to analyze method complexity. Each ticket must target CYC ≤8 (Jane Street strict). Output: $OUTPUT_FILE" \
    --context "$AUDIT_FILE" \
    --output "$OUTPUT_FILE" \
    2>&1 | tee "logs/wave6/phase4/$EPIC_ID.log"
