...
Create detailed extraction architecture for $EPIC_ID based on validated scope in $BOUNDARY_FILE. Query Jane Street KB for extraction patterns. Output: $OUTPUT_FILE
EOFMSG

# Step 3.2: Execute Bob CLI with command substitution
~/.npm-global/bin/bob --mode plan "$(cat /tmp/phase2_msg_$EPIC_ID.txt)" 2>&1 | tee "logs/wave6/phase2/$EPIC_ID.log"
