#!/bin/bash
# V12.52 Phase 4 Template: Ticket Generation
# Epic: {EPIC_ID}
# Agent: {AGENT_ID}
# Dependencies: Phase 3 (03-audit-report.md)
...
echo "=========================================="
echo "V12.52 Phase 4: Ticket Generation"
echo "Epic: $EPIC_ID"
echo "Agent: $AGENT_ID"
echo "=========================================="
...
echo "✅ V12.52 verification passed - proceeding with Phase 4"

# Step 2: Start Phase Execution (Record Lamport Event)
echo ""
echo "Step 2: Starting Phase 4 Execution"
echo "-----------------------------------"
...
echo "✅ Phase 4 started (Lamport event recorded)"

# Step 3: Execute Phase 4 Work (Ticket Generation)
echo ""
echo "Step 3: Executing Phase 4 Work"
echo "-----------------------------------"

# Read audit report
AUDIT_FILE="docs/brain/$EPIC_ID/03-audit-report.md"
if [ ! -f "$AUDIT_FILE" ]; then
    ERROR_MSG="Input file not found: $AUDIT_FILE"
    echo "❌ $ERROR_MSG"
    python3 scripts/epic_manifest.py fail_phase_execution "$EPIC_ID" "$PHASE" "$AGENT_ID" "$ERROR_MSG"
    exit 1
fi

# Generate tickets using Bob CLI (plan mode)
echo "Generating tickets for $EPIC_ID..."
OUTPUT_FILE="docs/brain/$EPIC_ID/04-tickets.md"

# Bob CLI command for ticket generation with jCodemunch complexity analysis
...
Generate surgical extraction tickets for $EPIC_ID based on audit report in $AUDIT_FILE. Use jCodemunch to analyze method complexity. Each ticket must target CYC ≤8 (Jane Street strict). Output: $OUTPUT_FILE
EOFMSG

# Step 3.2: Execute Bob CLI with command substitution
~/.npm-global/bin/bob --mode plan "$(cat /tmp/phase4_msg_$EPIC_ID.txt)" 2>&1 | tee "logs/wave6/phase4/$EPIC_ID.log"
...
echo "✅ Ticket generation complete: $OUTPUT_FILE"

# Step 4: Complete Phase Execution (Record Lamport Event)
echo ""
echo "Step 4: Completing Phase 4 Execution"
echo "-----------------------------------"
...
echo "✅ Phase 4 completed (Lamport event recorded)"

echo ""
echo "=========================================="
echo "✅ Phase 4 SUCCESS: $EPIC_ID"
