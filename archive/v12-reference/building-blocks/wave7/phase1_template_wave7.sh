...
# Step 3.2: Export API key from bashrc
export BOBSHELL_API_KEY=$(grep 'export BOBSHELL_API_KEY' ~/.bashrc | cut -d'=' -f2)

# Step 3.3: Execute Bob CLI with command substitution
~/.npm-global/bin/bob --yolo --chat-mode v12-phase1-scope "$(cat /tmp/phase1_msg_$EPIC_ID.txt)" 2>&1 | tee "logs/wave6/phase1/$EPIC_ID.log"
