...
cat > /tmp/phase6_msg_$EPIC_ID.txt << 'EOFMSG'
Perform final review for $EPIC_ID. Verify all ticket verifications in docs/brain/$EPIC_ID/. Run Greptile audit (expect 0 P0/P1 issues). Generate completion report. Output: $OUTPUT_FILE
EOFMSG

# Step 3.2: Execute Bob CLI with command substitution
~/.npm-global/bin/bob --mode advanced "$(cat /tmp/phase6_msg_$EPIC_ID.txt)" 2>&1 | tee "logs/wave6/phase6/$EPIC_ID.log"
