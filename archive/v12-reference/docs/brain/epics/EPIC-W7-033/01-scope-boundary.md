# EPIC-W7-033 — Phase 1.5: Scope Boundary Validation
...
**`FlattenSinglePosition` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `FlattenSinglePosition` in `src/V12_002.Orders.Management.Flatten.cs`
...
- `FlattenFilledMasterPositions` — confirmed not modified by this epic
- `FlattenAll` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `FlattenSinglePosition` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 27 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `FlattenSinglePosition`
...
- **Source analyzed:** `docs/brain/EPIC-W7-033/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `FlattenSinglePosition` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Orders.Management.Flatten.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > The **scope boundary** for EPIC-W7-033 Phase 1 is defined as: the body of the **single method** `FlattenSinglePosition` (lines 441–557, `src/V12_002.Orders.Management.Flatten.cs`) and the three new 
