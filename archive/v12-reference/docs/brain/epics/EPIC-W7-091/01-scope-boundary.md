# EPIC-W7-091 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-091/00-scope.md
...
**`CancelDirectFallbackOrders` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `CancelDirectFallbackOrders` in `src/V12_002.Safety.Watchdog.cs`
...
is internal to `CancelDirectFallbackOrders` only — extracting sub-logic into private helpers without
...
- **Target file:** `src/V12_002.Safety.Watchdog.cs`
...
are unaffected. The blast radius is contained to the single source file where `CancelDirectFallbackOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-091/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `CancelDirectFallbackOrders` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Safety.Watchdog.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `CancelDirectFallbackOrders` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `CancelDirectFallbackOrders` in `src/V12
