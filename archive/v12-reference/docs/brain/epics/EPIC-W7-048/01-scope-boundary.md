# EPIC-W7-048 — Phase 1.5: Scope Boundary Validation
...
**`UpdateExistingPendingReplacement` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `UpdateExistingPendingReplacement` in `src/V12_002.Trailing.StopUpdate.cs`
...
- `UpdateStopOrder` — confirmed not modified by this epic
- `HandleStalePendingReplacement` — confirmed not modified by this epic
- `CreateDirectStopOrder` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `UpdateExistingPendingReplacement` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `UpdateExistingPendingReplacement`
...
- **Source analyzed:** `docs/brain/EPIC-W7-048/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `UpdateExistingPendingReplacement` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Trailing.StopUpdate.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The scope boundary for this epic is **exactly one method**: `UpdateExistingPendingReplacement`.
