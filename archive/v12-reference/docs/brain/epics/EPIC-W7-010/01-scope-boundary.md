# EPIC-W7-010 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-010/00-scope.md
...
**`ShowModeSpecificControls` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ShowModeSpecificControls` in `src/V12_002.UI.Panel.Handlers.cs`
...
- `UpdateContextualUI` — confirmed not modified by this epic
- `PopulateDirectionCombo` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `ShowModeSpecificControls` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `ShowModeSpecificControls`
...
- **Source analyzed:** `docs/brain/EPIC-W7-010/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ShowModeSpecificControls` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.UI.Panel.Handlers.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** This epic covers exactly one unit of work: the body of the **single method**
