...
| Field | Value |
|-------|-------|
| **Method Name** | `SetRmaAnchorFromIpc` |
| **Current CYC** | 1 |
| **Target CYC** | <= 8 |
...
> **Only `SetRmaAnchorFromIpc` and its new extracted helper methods are in scope.**
...
within the same partial class file. The public/internal signature of `SetRmaAnchorFromIpc` must remain
...
**Thought 1**: `SetRmaAnchorFromIpc` in `src/V12_002.SIMA.cs` has CYC=1, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `SetRmaAnchorFromIpc` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `SetRmaAnchorFromIpc` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `SetRmaAnchorFromIpc`
- [x] Source file confirmed: `src/V12_002.SIMA.cs`
...
| **Output** | docs/brain/EPIC-W7-092/00-scope.md |
