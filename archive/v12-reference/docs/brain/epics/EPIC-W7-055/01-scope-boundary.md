# EPIC-W7-055 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-055/00-scope.md
...
**`DrainPhotonQueuesOnShutdown` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `DrainPhotonQueuesOnShutdown` in `src/V12_002.SIMA.Lifecycle.cs`
...
Direct caller count (from 00-scope.md analysis): **0**

- `ProcessShutdownSIMA` — confirmed not modified by this epic
- `CancelAllV12GtcOrders` — confirmed not modified by this epic
- `StopReaperAudit` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `DrainPhotonQueuesOnShutdown` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 8 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `DrainPhotonQueuesOnShutdown`
...
- **Source analyzed:** `docs/brain/EPIC-W7-055/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `DrainPhotonQueuesOnShutdown` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 0, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.SIMA.Lifecycle.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `DrainPhotonQueuesOnShutdown` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `DrainPhotonQueuesOnShutdown` in `src/V
