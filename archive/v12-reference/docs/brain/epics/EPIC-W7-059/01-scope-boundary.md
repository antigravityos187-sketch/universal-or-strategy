# EPIC-W7-059 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-059/00-scope.md
...
**`AdoptMasterWorkingOrders` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `AdoptMasterWorkingOrders` in `src/V12_002.SIMA.Lifecycle.cs (also in src-vm-backup/)`
...
Direct caller count (from 00-scope.md analysis): **2**

- `EnumerateApexAccounts` — confirmed not modified by this epic
- `ProcessOnConnectionStatusUpdate` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `AdoptMasterWorkingOrders` only — extracting sub-logic into private helpers without
...
- **Target file:** `src/V12_002.SIMA.Lifecycle.cs (also in src-vm-backup/)`
...
- **CYC baseline:** 34 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `AdoptMasterWorkingOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-059/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `AdoptMasterWorkingOrders` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 2, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.SIMA.Lifecycle.cs (also in src-vm-backup/)` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** is drawn around the `AdoptMasterWorkingOrders` logical cluster only.
Concretely, the following symbols fall inside the boundary:

- `HydrateWorkingOrdersFromBroker` — orchestrat
