# EPIC-W7-050 — Phase 1.5: Scope Boundary Validation
...
**`FleetSync_SyncFollowersToLevel` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `FleetSync_SyncFollowersToLevel` in `src/V12_002.Trailing.cs`
...
- `ManageTrail_RunFleetSymmetrySync` — confirmed not modified by this epic
- `UpdateStopOrder` — confirmed not modified by this epic
- `CalculateStopForLevel` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `FleetSync_SyncFollowersToLevel` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `FleetSync_SyncFollowersToLevel`
...
- **Source analyzed:** `docs/brain/EPIC-W7-050/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `FleetSync_SyncFollowersToLevel` present and in-scope per Phase 1 analysis
...
- **In scope:** the body of `FleetSync_SyncFollowersToLevel` and any private
  helper methods extracted *from* it during refactor (they do 
