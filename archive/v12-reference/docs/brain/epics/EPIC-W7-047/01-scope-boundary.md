# EPIC-W7-047 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-047/00-scope.md
...
**`CancelOrphanedTargets` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `CancelOrphanedTargets` in `src/V12_002.UI.Compliance.cs`
...
- `HandleFleetStopFill` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `CancelOrphanedTargets` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `CancelOrphanedTargets`
...
- **Source analyzed:** `docs/brain/EPIC-W7-047/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `CancelOrphanedTargets` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 0, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.UI.Compliance.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The scope boundary is defined as follows:

- **In scope:** the single method `CancelOrphanedTargets` (lines 553–578,
  `src/V12_002.UI.Compliance.cs`) and any private helper methods extracted directly
