# EPIC-W7-152 — Phase 1.5: Scope Boundary Validation
...
**`TryApplyConfigTarget_Value` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `TryApplyConfigTarget_Value` in `src/V12_002.UI.IPC.Commands.Config.cs`
...
is internal to `TryApplyConfigTarget_Value` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `TryApplyConfigTarget_Value`
...
- **Source analyzed:** `docs/brain/EPIC-W7-152/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `TryApplyConfigTarget_Value` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `TryApplyConfigTarget_Value` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `TryApplyConfigTarget_Value` in `src/V12
