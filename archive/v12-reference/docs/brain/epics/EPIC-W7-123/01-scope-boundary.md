# EPIC-W7-123 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-123/00-scope.md
...
**`HandleMatchedFollowerOrder` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `HandleMatchedFollowerOrder` in `src/V12_002.Orders.Callbacks.AccountOrders.cs`
...
is internal to `HandleMatchedFollowerOrder` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 14 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `HandleMatchedFollowerOrder`
...
- **Source analyzed:** `docs/brain/EPIC-W7-123/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `HandleMatchedFollowerOrder` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 3, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Orders.Callbacks.AccountOrders.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `HandleMatchedFollowerOrder` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `HandleMatchedFollowerOrder` in `src/V12
