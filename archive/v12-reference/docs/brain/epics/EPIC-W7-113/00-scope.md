...
**Thought 1**: `HydrateFSMsFromWorkingOrders` in `src/V12_002.SIMA.Lifecycle.cs` has CYC=12, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `HydrateFSMsFromWorkingOrders` only -- no scope creep.
...
| **Output** | docs/brain/EPIC-W7-113/00-scope.md |
