...
> **Only `SymmetryGuardSubmitFollowerBracket` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `SymmetryGuardSubmitFollowerBracket` in `src/V12_002.Symmetry.Follower.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `SymmetryGuardSubmitFollowerBracket` must remain
...
**Thought 1**: `SymmetryGuardSubmitFollowerBracket` in `src/V12_002.Symmetry.Follower.cs` has CYC=16, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `SymmetryGuardSubmitFollowerBracket` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `SymmetryGuardSubmitFollowerBracket` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `SymmetryGuardSubmitFollowerBracket`
...
| **Output** | docs/brain/EPIC-W7-126/00-scope.md |
