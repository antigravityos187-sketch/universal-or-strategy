# EPIC-W7-015 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-015/00-scope.md
...
**`CancelAll_ProcessSingleFleetAccount` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `CancelAll_ProcessSingleFleetAccount` in `src/V12_002.UI.IPC.Commands.Fleet.cs`
...
- `CancelAll_ProcessFleetOrders` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `CancelAll_ProcessSingleFleetAccount` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 18 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `CancelAll_ProcessSingleFleetAccount`
...
- **Source analyzed:** `docs/brain/EPIC-W7-015/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `CancelAll_ProcessSingleFleetAccount` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 2, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.UI.IPC.Commands.Fleet.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for EPIC-W7-015 is defined as follows:

- **Inside scope:** The body of `CancelAll_ProcessSingleFleetAccount` (lines 300–343)
  and any new private helper methods that may be ex
