# EPIC-W7-069 — Phase 1.5: Scope Boundary Validation
...
**`GetFsmExpectedPosition` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `GetFsmExpectedPosition` in `src/V12_002.Symmetry.BracketFSM.cs`
...
is internal to `GetFsmExpectedPosition` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `GetFsmExpectedPosition`
...
- **Source analyzed:** `docs/brain/EPIC-W7-069/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `GetFsmExpectedPosition` present and in-scope per Phase 1 analysis
