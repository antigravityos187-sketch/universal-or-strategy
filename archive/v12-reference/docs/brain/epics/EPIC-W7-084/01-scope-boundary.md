# EPIC-W7-084 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-084/00-scope.md
...
**`AuditFleet_CalculateExpectedActual` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `AuditFleet_CalculateExpectedActual` in `src/V12_002.REAPER.Audit.cs`
...
is internal to `AuditFleet_CalculateExpectedActual` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 382 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `AuditFleet_CalculateExpectedActual`
...
- **Source analyzed:** `docs/brain/EPIC-W7-084/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `AuditFleet_CalculateExpectedActual` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `AuditFleet_CalculateExpectedActual` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `AuditFleet_CalculateExpectedAct
