# EPIC-W7-090 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-090/00-scope.md
...
**`OnWatchdogTimer` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `OnWatchdogTimer` in `src/V12_002.Safety.Watchdog.cs`
...
is internal to `OnWatchdogTimer` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 244 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `OnWatchdogTimer`
...
- **Source analyzed:** `docs/brain/EPIC-W7-090/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `OnWatchdogTimer` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `OnWatchdogTimer` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `OnWatchdogTimer` in `src/V12_002.Safety.Watchdog.c
