# EPIC-W7-129 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-129/00-scope.md
...
**`SymmetryGuardTryResolveFollower` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SymmetryGuardTryResolveFollower` in `src/V12_002.Symmetry.Follower.cs`
...
Direct caller count (from 00-scope.md analysis): **5**
...
is internal to `SymmetryGuardTryResolveFollower` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 16 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `SymmetryGuardTryResolveFollower`
...
- **Source analyzed:** `docs/brain/EPIC-W7-129/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SymmetryGuardTryResolveFollower` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 5, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Symmetry.Follower.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `SymmetryGuardTryResolveFollower` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `SymmetryGuardTryResolveFollower` i
