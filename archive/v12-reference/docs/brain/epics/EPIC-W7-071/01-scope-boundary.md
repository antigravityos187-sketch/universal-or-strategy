# EPIC-W7-071 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-071/00-scope.md
...
**`ShadowProcessFollowerStopUpdate` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `ShadowProcessFollowerStopUpdate` in `src/V12_002.SIMA.Shadow.cs`
...
is internal to `ShadowProcessFollowerStopUpdate` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `ShadowProcessFollowerStopUpdate`
...
- **Source analyzed:** `docs/brain/EPIC-W7-071/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `ShadowProcessFollowerStopUpdate` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 1, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.SIMA.Shadow.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `ShadowProcessFollowerStopUpdate` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ShadowProcessFollowerStopUpdate` i
