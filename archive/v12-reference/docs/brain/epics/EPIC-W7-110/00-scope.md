...
> **Only `AdoptMasterOrders` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `AdoptMasterOrders` in `src/V12_002.SIMA.Lifecycle.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `AdoptMasterOrders` must remain
...
- **Current CYC**: 22 -- exceeds Jane Street strict threshold of 8
- **Target CYC**: <= 8
- **Reduction required**: 14 complexity points to extract
...
**Thought 1**: `AdoptMasterOrders` in `src/V12_002.SIMA.Lifecycle.cs` has CYC=22, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `AdoptMasterOrders` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `AdoptMasterOrders` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `AdoptMasterOrders`
- [x] Source file confirmed: `src/V12_002.SIMA.Lifecycle.cs`
- [x] CYC verified: 22 (target <= 8)
...
| **Output** | docs/brain/EPIC-W7-110/00-scope.md |
