# EPIC-W7-037 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-037/00-scope.md
...
**`SymmetryNormalizeTradeType` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SymmetryNormalizeTradeType` in `src/V12_002.Symmetry.Replace.cs`
...
Direct caller count (from 00-scope.md analysis): **12**

- `SymmetryInferTradeType` — confirmed not modified by this epic
- `SymmetryGuardBeginDispatch` — confirmed not modified by this epic
- `SymmetryFindDispatchForMasterFill` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `SymmetryNormalizeTradeType` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `SymmetryNormalizeTradeType`
...
- **Source analyzed:** `docs/brain/EPIC-W7-037/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SymmetryNormalizeTradeType` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 12, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Symmetry.Replace.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for this epic is strictly limited to `SymmetryNormalizeTradeType` and the
