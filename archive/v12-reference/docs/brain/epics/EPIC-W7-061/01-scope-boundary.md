# EPIC-W7-061 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-061/00-scope.md
...
**`SubmitAndRegisterFleetOrders` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SubmitAndRegisterFleetOrders` in `src/V12_002.SIMA.Fleet.cs`
...
- `ProcessFleetSlot` — confirmed not modified by this epic
...
is internal to `SubmitAndRegisterFleetOrders` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `SubmitAndRegisterFleetOrders`
...
- **Source analyzed:** `docs/brain/EPIC-W7-061/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SubmitAndRegisterFleetOrders` present and in-scope per Phase 1 analysis
