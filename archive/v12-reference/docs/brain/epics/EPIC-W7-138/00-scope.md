...
> **Only `ManageTrail_RunPerTradeBranches` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ManageTrail_RunPerTradeBranches` in `src/V12_002.Trailing.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `ManageTrail_RunPerTradeBranches` must remain
...
**Thought 1**: `ManageTrail_RunPerTradeBranches` in `src/V12_002.Trailing.cs` has CYC=1, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `ManageTrail_RunPerTradeBranches` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `ManageTrail_RunPerTradeBranches` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.
...
| **Output** | docs/brain/EPIC-W7-138/00-scope.md |
