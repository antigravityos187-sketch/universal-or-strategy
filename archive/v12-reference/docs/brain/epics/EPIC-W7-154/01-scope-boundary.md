# EPIC-W7-154 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-154/00-scope.md
...
**`TryHandleFleet_LongShort` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `TryHandleFleet_LongShort` in `src/V12_002.UI.IPC.Commands.Fleet.cs`
...
is internal to `TryHandleFleet_LongShort` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `TryHandleFleet_LongShort`
...
- **Source analyzed:** `docs/brain/EPIC-W7-154/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `TryHandleFleet_LongShort` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `TryHandleFleet_LongShort` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `TryHandleFleet_LongShort` in `src/V12_002
