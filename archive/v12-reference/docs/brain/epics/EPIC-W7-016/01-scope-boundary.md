# EPIC-W7-016 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-016/00-scope.md
...
**`TryHandleFleet_CancelAll` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `TryHandleFleet_CancelAll` in `src/V12_002.UI.IPC.Commands.Fleet.cs`
...
is internal to `TryHandleFleet_CancelAll` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 21 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `TryHandleFleet_CancelAll`
...
- **Source analyzed:** `docs/brain/EPIC-W7-016/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `TryHandleFleet_CancelAll` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** The **scope boundary** is drawn precisely around `TryHandleFleet_CancelAll` (lines 177–232 of `src/V12_002.UI.IPC.Commands.Fleet.cs`). Everything outside that method body is out of scope for Phase 1.
