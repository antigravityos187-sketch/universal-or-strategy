# EPIC-W7-031 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-031/00-scope.md
...
**`AuditMaster_HandleNakedPosition` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `AuditMaster_HandleNakedPosition` in `src/V12_002.REAPER.Audit.cs`
...
- `AuditMasterAccountIfNeeded` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `AuditMaster_HandleNakedPosition` only — extracting sub-logic into private helpers without
...
- **Target file:** `src/V12_002.REAPER.Audit.cs`
...
- **CYC baseline:** 19 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `AuditMaster_HandleNakedPosition`
...
- **Source analyzed:** `docs/brain/EPIC-W7-031/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `AuditMaster_HandleNakedPosition` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 2, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.REAPER.Audit.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** The **scope boundary** for this epic is defined as follows:

- **In scope:** the body of `AuditMaster_HandleNakedPosition` (lines 624–679) and up to
  three private helper methods that will be extract
