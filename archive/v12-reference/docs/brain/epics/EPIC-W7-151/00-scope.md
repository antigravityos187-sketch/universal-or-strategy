...
**Thought 1**: `IsOrderAllowed` in `src/V12_002.UI.Compliance.cs` has CYC=9, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `IsOrderAllowed` only -- no scope creep.
...
| **Output** | docs/brain/EPIC-W7-151/00-scope.md |
