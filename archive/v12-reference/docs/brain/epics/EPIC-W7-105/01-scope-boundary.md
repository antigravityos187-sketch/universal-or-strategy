# EPIC-W7-105 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-105/00-scope.md
...
**`DrainAllDispatchQueuesOnAbort` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `DrainAllDispatchQueuesOnAbort` in `src/V12_002.SIMA.Fleet.cs`
...
is internal to `DrainAllDispatchQueuesOnAbort` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `DrainAllDispatchQueuesOnAbort`
...
- **Source analyzed:** `docs/brain/EPIC-W7-105/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `DrainAllDispatchQueuesOnAbort` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** > **Only `DrainAllDispatchQueuesOnAbort` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `DrainAllDispatchQueuesOnAbort` in `s
