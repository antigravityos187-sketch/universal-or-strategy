...
**Thought 1**: `AuditFleet_CalculateExpectedActual` in `src/V12_002.REAPER.Audit.cs` has CYC=3, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `AuditFleet_CalculateExpectedActual` only -- no scope creep.
...
| **Output** | docs/brain/EPIC-W7-116/00-scope.md |
