...
> **Only `ClassifyOrderByPrefix` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ClassifyOrderByPrefix` in `src/V12_002.SIMA.Lifecycle.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `ClassifyOrderByPrefix` must remain
...
**Thought 1**: `ClassifyOrderByPrefix` in `src/V12_002.SIMA.Lifecycle.cs` has CYC=20, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `ClassifyOrderByPrefix` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `ClassifyOrderByPrefix` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `ClassifyOrderByPrefix`
...
| **Output** | docs/brain/EPIC-W7-112/00-scope.md |
