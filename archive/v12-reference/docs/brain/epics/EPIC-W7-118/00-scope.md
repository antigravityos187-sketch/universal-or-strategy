...
**Thought 1**: `DeserializeSnapshot` in `src/V12_002.StickyState.cs` has CYC=0, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `DeserializeSnapshot` only -- no scope creep.
...
| **Output** | docs/brain/EPIC-W7-118/00-scope.md |
