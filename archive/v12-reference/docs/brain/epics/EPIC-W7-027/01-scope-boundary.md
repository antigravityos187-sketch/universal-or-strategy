# EPIC-W7-027 — Phase 1.5: Scope Boundary Validation
...
**`Dispatch_PublishMarketBracketToPhoton` + 2 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `Dispatch_PublishMarketBracketToPhoton` in `src/V12_002.SIMA.Dispatch.cs`
...
- `Dispatch_ProcessFleetLoop` — confirmed not modified by this epic

The caller signature, behavior, and call sites are not modified by this epic. The refactor
is internal to `Dispatch_PublishMarketBracketToPhoton` only — extracting sub-logic into private helpers without
...
are unaffected. The blast radius is contained to the single source file where `Dispatch_PublishMarketBracketToPhoton`
...
- **Source analyzed:** `docs/brain/EPIC-W7-027/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `Dispatch_PublishMarketBracketToPhoton` present and in-scope per Phase 1 analysis
...
- **Scope statement from Phase 1:** The **scope boundary** is drawn at the declaration of `Dispatch_PublishMarketBracketToPhoton`
