# EPIC-W7-128 — Phase 1.5: Scope Boundary Validation
...
**Input:** docs/brain/EPIC-W7-128/00-scope.md
...
**`SymmetryGuardReplaceExistingFollowerTarget` + 5 new helper methods (to be named in Phase 2)**

The scope is strictly limited to:
1. The target method: `SymmetryGuardReplaceExistingFollowerTarget` in `src/V12_002.Symmetry.Replace.cs`
...
Direct caller count (from 00-scope.md analysis): **6**
...
is internal to `SymmetryGuardReplaceExistingFollowerTarget` only — extracting sub-logic into private helpers without
...
- **CYC baseline:** 20 (target: <= 8 after extraction)
...
are unaffected. The blast radius is contained to the single source file where `SymmetryGuardReplaceExistingFollowerTarget`
...
- **Source analyzed:** `docs/brain/EPIC-W7-128/00-scope.md` (Phase 1 output)
- **Symbol confirmed:** `SymmetryGuardReplaceExistingFollowerTarget` present and in-scope per Phase 1 analysis
- **References analyzed:** Caller count = 6, all confirmed upstream-only
- **Blast radius:** Limited to `src/V12_002.Symmetry.Replace.cs` + private helpers (same partial class)
- **Scope statement from Phase 1:** > **Only `SymmetryGuardReplaceExistingFollowerTarget` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `SymmetryGuardReplaceExi
