...
| Field | Value |
|-------|-------|
| **Method Name** | `ProcessSingleFleetRMAAccount` |
| **Current CYC** | 12 |
| **Target CYC** | <= 8 |
...
> **Only `ProcessSingleFleetRMAAccount` and its new extracted helper methods are in scope.**

This epic covers exclusively the cyclomatic complexity reduction of `ProcessSingleFleetRMAAccount` in `src/V12_002.SIMA.Execution.cs`.
No other methods will be modified. No cross-file changes beyond adding `private` helper methods
within the same partial class file. The public/internal signature of `ProcessSingleFleetRMAAccount` must remain
...
**Thought 1**: `ProcessSingleFleetRMAAccount` in `src/V12_002.SIMA.Execution.cs` has CYC=12, which exceeds the Jane Street threshold of <=8. The scope must be strictly limited to extracting helper methods from `ProcessSingleFleetRMAAccount` only -- no scope creep.
...
**Thought 3**: Scope boundary confirmed. Only `ProcessSingleFleetRMAAccount` and its newly extracted private helper methods are in scope. scope_confirmed_single_method = true.

---

## Scope Checklist

- [x] Single method identified: `ProcessSingleFleetRMAAccount`
...
| **Output** | docs/brain/EPIC-W7-095/00-scope.md |
