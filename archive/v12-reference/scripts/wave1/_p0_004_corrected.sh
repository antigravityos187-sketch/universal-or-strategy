...
mkdir -p docs/brain/EPIC-004
mkdir -p logs/phase0

cat > /tmp/phase0_msg_004.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-004.
...
## Target Methods (3 methods in same file)
- Method 1: Dispatch_PublishMarketBracketToPhoton (CYC=21, LOC=189)
- Method 2: Dispatch_ProcessFleetLoop (CYC=14, LOC=113)
- Method 3: Dispatch_PublishLimitEntryToPhoton (CYC=11, LOC=95)
- File: src/V12_002.SIMA.Dispatch.cs
- Complexity: 21, 14, 11 (target: all ≤8)
...
2. get_blast_radius(repo='universal-or-strategy', symbol='Dispatch_PublishMarketBracketToPhoton')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='Dispatch_PublishMarketBracketToPhoton')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='Dispatch_PublishMarketBracketToPhoton')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-004/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-004/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-004/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-004/manifest.json << 'EOF'
{
  "epic_id": "EPIC-004",
  "methods": ["Dispatch_PublishMarketBracketToPhoton", "Dispatch_ProcessFleetLoop", "Dispatch_PublishLimitEntryToPhoton"],
  "file": "src/V12_002.SIMA.Dispatch.cs",
  "complexity": [21, 14, 11],
  "phases": {
    "0": {
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-004/00-hotspots.md && wc -l docs/brain/EPIC-004/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-004/manifest.json && cat docs/brain/EPIC-004/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_004.txt)" 2>&1 | tee logs/phase0/EPIC-004.log
echo "DONE_EXIT=$?"
