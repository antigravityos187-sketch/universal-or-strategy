...
Execute Phase 1 (Scope Definition) for EPIC-001.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-001/00-scope.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-001/00-scope.md << 'EOF'
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-001/00-scope.md && wc -l docs/brain/EPIC-001/00-scope.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_003.txt)" 2>&1 | tee logs/phase1/EPIC-001.log
echo "DONE_EXIT=$?"
