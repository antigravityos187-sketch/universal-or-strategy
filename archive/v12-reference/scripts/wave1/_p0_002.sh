...
mkdir -p docs/brain/EPIC-002
mkdir -p logs/phase0

cat > /tmp/phase0_msg_002.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-002.
...
- Method 1: HandleFlatPosition_CleanupActivePositions
- Method 2: HandleFlatPosition_UpdateState
- File: src/V12_002.Orders.Callbacks.Execution.cs
- Complexity: 17, 10 (target: all ≤8)
...
2. get_blast_radius(repo='universal-or-strategy', symbol='HandleFlatPosition_CleanupActivePositions')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='HandleFlatPosition_CleanupActivePositions')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='HandleFlatPosition_CleanupActivePositions')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-002/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-002/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-002/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-002/manifest.json << 'EOF'
{
  "epic_id": "EPIC-002",
  "methods": ["HandleFlatPosition_CleanupActivePositions", "HandleFlatPosition_UpdateState"],
  "file": "src/V12_002.Orders.Callbacks.Execution.cs",
  "complexity": [17, 10],
  "phases": {
    "0": {
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-002/00-hotspots.md && wc -l docs/brain/EPIC-002/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-002/manifest.json && cat docs/brain/EPIC-002/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_002.txt)" 2>&1 | tee logs/phase0/EPIC-002.log
echo "DONE_EXIT=$?"
