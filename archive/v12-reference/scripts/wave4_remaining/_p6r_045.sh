#!/bin/bash
# Phase 6 (Verification) for EPIC-CCN-045
set -e
...
if ! find docs/brain/EPIC-CCN-045 -maxdepth 1 \( -name "05-*.md" -o -name "ticket-*-completion.md" \) -print -quit | grep -q .; then
    echo "ERROR: Missing Phase 5 completion files for EPIC-CCN-045"
    echo "Expected: docs/brain/EPIC-CCN-045/05-*.md OR ticket-*-completion.md"
    exit 1
fi
...
if [ -f "docs/brain/EPIC-CCN-045/06-completion-report.md" ]; then
    ...
    echo "ERROR: No completion report created for EPIC-CCN-045"
    exit 1
fi
