...
# Epic to API key mapping (same as Phase 3)
API_ALLOCATION = {
    "107": "b (2).json",
    "108": "b.json",
    "109": "bob (1).json",
...
def generate_phase4_script(epic_num):
    """Generate Phase 4 script for given epic number"""
...
You are executing Phase 4 (Ticket Generation) for EPIC-CCN-{epic_num}.

**Input Artifacts**: 
- Read `docs/brain/EPIC-CCN-{epic_num}/02-architecture-plan.md` for extraction plan
- Read `docs/brain/EPIC-CCN-{epic_num}/03-audit-report.md` for audit results

**Your Task**: Generate detailed implementation tickets for surgical extraction.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-{epic_num}/04-tickets.md` with:
...
bob --yolo --chat-mode plan "$(cat /tmp/phase4_msg_{epic_num}.txt)" 2>&1 | tee logs/phase4/EPIC-CCN-{epic_num}.log
echo "DONE_EXIT=$?"

# Made with Bob
"""
    
    filename = f"_p4_{epic_num}.sh"
...
"""
    ...
    with open("launch_phase4_all_screen.sh", 'w', newline='\n') as f:
        f.write(launcher)
    
    print("[OK] Created launch_phase4_all_screen.sh")

def main():
    print("=" * 60)
    print("Phase 4 Script Generator (Ticket Generation)")
    print("=" * 60)
    print()
    ...
    print("   gcloud compute scp _p4_*.sh v12-test-golden-v2:~/universal-or-strategy/ --zone=us-central1-a")
    print("   gcloud compute scp launch_phase4_all_screen.sh v12-test-golden-v2:~/universal-or-strategy/ --zone=us-central1-a")
    print()
