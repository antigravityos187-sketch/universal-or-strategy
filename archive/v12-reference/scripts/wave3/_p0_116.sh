...
cat > /tmp/phase0_msg_116.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-116.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-116/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-116/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-116/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-116/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-116",
  "method": "HandleFlatPosition_CleanupActivePositions",
  "file": "src/V12_002.Orders.Callbacks.Execution.cs",
  "complexity": 17,
  "phases": {
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-116/00-hotspots.md && wc -l docs/brain/EPIC-CCN-116/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-116/manifest.json && cat docs/brain/EPIC-CCN-116/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_116.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-116.log
echo "DONE_EXIT=$?"
