...
cat > /tmp/phase4_msg_118.txt << 'EOFMSG'
You are executing Phase 4 (Ticket Generation) for EPIC-CCN-118.

**Input Artifacts**: 
- Read `docs/brain/EPIC-CCN-118/02-architecture-plan.md` for extraction plan
- Read `docs/brain/EPIC-CCN-118/03-audit-report.md` for audit results

**Your Task**: Generate detailed implementation tickets for surgical extraction.

**Output Requirements**:
1. Create `docs/brain/EPIC-CCN-118/04-tickets.md` with:
...
bob --yolo --chat-mode plan "$(cat /tmp/phase4_msg_118.txt)" 2>&1 | tee logs/phase4/EPIC-CCN-118.log
echo "DONE_EXIT=$?"
