...
# Output: docs/brain/EPIC-CCN-040/01-scope-boundary.md
...
can_execute, reason = verify_can_execute('$EPIC_ID', '$PHASE', '$tory')
if not can_execute:
...
started, reason = start_phase_execution('$EPIC_ID', '$PHASE', '$tory')
if not started:
...
recorded, reason = fail_phase_execution('$EPIC_ID', '$PHASE', '$tory', '$ERROR_MSG')
