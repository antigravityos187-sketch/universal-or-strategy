#!/bin/bash
# Phase 3 (DNA & PR Audit) for EPIC-CCN-026
...
cat > /tmp/phase3_msg_026.txt << 'EOFMSG'
Use the phase-3-audit MCP server to execute Phase 3 for EPIC-CCN-026.

Call the execute_phase_3 tool with epic_id="EPIC-CCN-026".
...
bob --yolo "$(cat /tmp/phase3_msg_026.txt)"

# Verify file created
if [ -f "docs/brain/EPIC-CCN-026/03-audit-report.md" ]; then
    echo "SUCCESS: Phase 3 complete for EPIC-CCN-026"
    echo "File: docs/brain/EPIC-CCN-026/03-audit-report.md"
    ls -lh "docs/brain/EPIC-CCN-026/03-audit-report.md"
else
    echo "ERROR: File not created for EPIC-CCN-026"
    exit 1
fi
