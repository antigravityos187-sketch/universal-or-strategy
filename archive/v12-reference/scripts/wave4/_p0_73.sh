...
mkdir -p docs/brain/EPIC-CCN-73
mkdir -p logs/phase0

cat > /tmp/phase0_msg_73.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-73.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-73/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-73/00-hotspots.md << 'EOF'
# Phase 0: Hotspot Analysis - EPIC-CCN-73
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-73/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-73/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-73",
  "method": "RestoreCascadedTargets",
  "file": "src/V12_002.Orders.Management.StopSync.cs",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-73/00-hotspots.md && wc -l docs/brain/EPIC-CCN-73/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-73/manifest.json && cat docs/brain/EPIC-CCN-73/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
screen -dmS p0-73 bash -l -c "bob --yolo --chat-mode v12-phase0-hotspot \"$(cat /tmp/phase0_msg_73.txt)\" 2>&1 | tee logs/phase0/EPIC-CCN-73.log"
echo "DONE_EXIT=$?"
