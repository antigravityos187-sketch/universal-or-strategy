...
mkdir -p docs/brain/EPIC-CCN-030
mkdir -p logs/phase0

cat > /tmp/phase0_msg_030.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-030.
...
2. Filter violations by file: src/V12_002.Orders.Management.Cleanup.cs
...
<execute_command>
<command>
grep -i "V12_002.Orders.Management.Cleanup.cs" jane_street_p0_violations.json | wc -l
</command>
...
2. get_blast_radius(repo='universal-or-strategy', symbol='ValidateOrphanedMasterOrders')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='ValidateOrphanedMasterOrders')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='ValidateOrphanedMasterOrders')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-030/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-030/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-030/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-030/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-030",
  "method": "ValidateOrphanedMasterOrders",
  "file": "src/V12_002.Orders.Management.Cleanup.cs",
  "complexity": 19,
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-030/00-hotspots.md && wc -l docs/brain/EPIC-CCN-030/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-030/manifest.json && cat docs/brain/EPIC-CCN-030/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
bob --yolo --chat-mode v12-phase0-hotspot "$(cat /tmp/phase0_msg_030.txt)" 2>&1 | tee logs/phase0/EPIC-CCN-030.log
echo "DONE_EXIT=$?"
