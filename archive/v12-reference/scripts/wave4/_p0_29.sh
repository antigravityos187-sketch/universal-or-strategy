...
mkdir -p docs/brain/EPIC-CCN-29
mkdir -p logs/phase0

cat > /tmp/phase0_msg_29.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-29.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='ExecuteRetestEntry')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='ExecuteRetestEntry')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='ExecuteRetestEntry')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-29/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-29/00-hotspots.md << 'EOF'
# Phase 0: Hotspot Analysis - EPIC-CCN-29
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-29/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-29/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-29",
  "method": "ExecuteRetestEntry",
  "file": "src/V12_002.Entries.Retest.cs",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-29/00-hotspots.md && wc -l docs/brain/EPIC-CCN-29/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-29/manifest.json && cat docs/brain/EPIC-CCN-29/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
screen -dmS p0-29 bash -l -c "bob --yolo --chat-mode v12-phase0-hotspot \"$(cat /tmp/phase0_msg_29.txt)\" 2>&1 | tee logs/phase0/EPIC-CCN-29.log"
echo "DONE_EXIT=$?"
