...
cat > /tmp/phase1_msg_027.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition + Boundary Validation) for EPIC-CCN-027.
...
**Input**: Read `docs/brain/EPIC-CCN-027/00-hotspots.md`
...
Create `docs/brain/EPIC-CCN-027/01-scope.md` with:
...
2. **Boundary Definition**:
   - What's IN scope: Dispatch_PublishMarketBracketToPhoton body only
...
Create `docs/brain/EPIC-CCN-027/01-scope-boundary.md` with:

1. **Boundary Check**:
   - ✅ Scope limited to single method: Dispatch_PublishMarketBracketToPhoton
...
cat > docs/brain/EPIC-CCN-027/01-scope.md << 'EOF'
[Your scope definition content here]
EOF

# Create 01-scope-boundary.md
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat > docs/brain/EPIC-CCN-027/01-scope-boundary.md << 'EOF'
[Your boundary validation content here]
EOF

# Verify files
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-027/01-scope.md && wc -l docs/brain/EPIC-CCN-027/01-scope.md

execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-027/01-scope-boundary.md && wc -l docs/brain/EPIC-CCN-027/01-scope-boundary.md

# Update manifest
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat docs/brain/EPIC-CCN-027/manifest.json
```
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_027.txt)" 2>&1 | tee logs/phase1/EPIC-CCN-027.log
echo "DONE_EXIT=$?"
