...
cat > /tmp/phase1_msg_014.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition + Boundary Validation) for EPIC-CCN-014.
...
**Input**: Read `docs/brain/EPIC-CCN-014/00-hotspots.md`
...
Create `docs/brain/EPIC-CCN-014/01-scope.md` with:
...
2. **Boundary Definition**:
   - What's IN scope: TryHandleFleetCommand body only
...
Create `docs/brain/EPIC-CCN-014/01-scope-boundary.md` with:

1. **Boundary Check**:
   - ✅ Scope limited to single method: TryHandleFleetCommand
...
cat > docs/brain/EPIC-CCN-014/01-scope.md << 'EOF'
[Your scope definition content here]
EOF

# Create 01-scope-boundary.md
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat > docs/brain/EPIC-CCN-014/01-scope-boundary.md << 'EOF'
[Your boundary validation content here]
EOF

# Verify files
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-014/01-scope.md && wc -l docs/brain/EPIC-CCN-014/01-scope.md

execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-014/01-scope-boundary.md && wc -l docs/brain/EPIC-CCN-014/01-scope-boundary.md
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_014.txt)" 2>&1 | tee logs/phase1/EPIC-CCN-014.log
echo "DONE_EXIT=$?"
