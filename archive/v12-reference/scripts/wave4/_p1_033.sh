...
mkdir -p docs/brain/EPIC-CCN-033
mkdir -p logs/phase1

cat > /tmp/phase1_msg_033.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition + Boundary Validation) for EPIC-CCN-033.
...
**Input**: Read `docs/brain/EPIC-CCN-033/00-hotspots.md`
...
Create `docs/brain/EPIC-CCN-033/01-scope.md` with:
...
Create `docs/brain/EPIC-CCN-033/01-scope-boundary.md` with:

1. **Boundary Check**:
   - ✅ Scope limited to single method: FlattenSinglePosition
   - ✅ No changes to callers
   - ✅ No changes to callees
   - ✅ No changes to other methods in V12_002.Orders.Management.Flatten.cs
...
cat > docs/brain/EPIC-CCN-033/01-scope.md << 'EOF'
[Your scope definition content here]
EOF

# Create 01-scope-boundary.md
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat > docs/brain/EPIC-CCN-033/01-scope-boundary.md << 'EOF'
[Your boundary validation content here]
EOF

# Verify files
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-033/01-scope.md && wc -l docs/brain/EPIC-CCN-033/01-scope.md

execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-033/01-scope-boundary.md && wc -l docs/brain/EPIC-CCN-033/01-scope-boundary.md

# Update manifest
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat docs/brain/EPIC-CCN-033/manifest.json
```
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_033.txt)" 2>&1 | tee logs/phase1/EPIC-CCN-033.log
echo "DONE_EXIT=$?"
