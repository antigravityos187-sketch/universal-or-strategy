#!/usr/bin/env python3
"""
Generate Phase 3 (DNA & PR Audit) scripts for Wave 4 using building-blocks method.

CRITICAL: This script uses the building-blocks method - it copies Phase 2 scripts
and modifies only phase-specific parameters. NEVER generate from scratch.

Reference: docs/workflow/WAVE_PHASE_SCRIPT_GENERATION_SOP_V3.md
"""

import json
import os
from pathlib import Path

# Load epic roadmap
roadmap_path = Path("epic_roadmap_wave4_fresh.json")
if not roadmap_path.exists():
    print(f"ERROR: {roadmap_path} not found!")
    exit(1)

with open(roadmap_path, 'r', encoding='utf-8-sig') as f:
    content = f.read().strip()
    if not content:
        print(f"ERROR: {roadmap_path} is empty!")
        exit(1)
    epics_data = json.loads(content)
    # Handle both array and object formats
    if isinstance(epics_data, list):
        epics = epics_data
    else:
        epics = epics_data.get("epics", [])
# Load API keys
api_dir = Path("docs/API")
api_files = sorted(api_dir.glob("*.json"))[:15]  # Use first 15 APIs

# Exhausted API to skip (from pilot test failure)
EXHAUSTED_API = "bob_prod_bob-admin_V7HJU1JXC5q7bLKAr7o8nYQMwWb3uLVj6U8b3FYjkbDzzYaccrZX5E7U9pxZxTBoiz2xTv7FGBtSW5QaTZppUzr_FFZsSht5Ab1MM5H97Z4jcfTweD36Ym7i11JATwHMbAvu"

api_keys = []
for api_file in api_files:
    with open(api_file) as f:
        data = json.load(f)
        api_key = data["apikey"]
        # Skip exhausted API
        if api_key != EXHAUSTED_API:
            api_keys.append(api_key)
        else:
            print(f"Skipping exhausted API from {api_file.name}")

print(f"Loaded {len(api_keys)} healthy API keys (1 exhausted API excluded)")

# Generate Phase 3 scripts (copy Phase 2 pattern)
output_dir = Path("scripts/wave4")
output_dir.mkdir(parents=True, exist_ok=True)

for i, epic in enumerate(epics, 1):
    epic_id = epic.get("epic_id") or epic.get("epic_number")
    epic_num = epic_id.split("-")[-1]  # Extract "001" from "EPIC-CCN-001"
    
    # Round-robin API assignment
    api_index = (i - 1) % len(api_keys)
    api_key = api_keys[api_index]
    
    # Phase 3 script (uses phase-3-audit MCP tool)
    script_content = f'''#!/bin/bash
# Phase 3 (DNA & PR Audit) for {epic_id}
# Generated: 2026-06-15
# Method: MCP tool (phase-3-audit server)

set -e

EPIC_ID="{epic_id}"
API_KEY="{api_key}"

# Export API key
export BOBSHELL_API_KEY="$API_KEY"

# Create message file (avoids bash multi-line escaping)
cat > /tmp/phase3_msg_{epic_num}.txt << 'EOFMSG'
Use the phase-3-audit MCP server to execute Phase 3 for {epic_id}.

Call the execute_phase_3 tool with epic_id="{epic_id}".

The tool will return complete instructions for DNA & PR audit.
Follow those instructions to create the audit report.

**Verification**: Confirm file exists on disk before reporting success.

**Bobcoin Tracking**: Report usage in format "Cost: X.XX | Balance: Y.YY"
EOFMSG

# Execute with Bob Shell (uses phase-3-audit MCP tool)
bob --yolo "$(cat /tmp/phase3_msg_{epic_num}.txt)"

# Verify file created
if [ -f "docs/brain/{epic_id}/03-audit-report.md" ]; then
    echo "SUCCESS: Phase 3 complete for {epic_id}"
    echo "File: docs/brain/{epic_id}/03-audit-report.md"
    ls -lh "docs/brain/{epic_id}/03-audit-report.md"
else
    echo "ERROR: File not created for {epic_id}"
    exit 1
fi
'''
    
    # Write script
    script_path = output_dir / f"_p3_{epic_num}.sh"
    with open(script_path, 'w', newline='\n') as f:  # Force LF line endings
        f.write(script_content)
    
    # Make executable
    os.chmod(script_path, 0o755)
    
    print(f"Generated: {script_path}")

# Generate test launcher (first 2 epics)
test_launcher = '''#!/bin/bash
# Phase 3 Test Launcher - First 2 Epics
# Generated: 2026-06-15

set -e

echo "[$(date)] Starting Phase 3 test launch (2 epics)"

# Create logs directory
mkdir -p logs/phase3

# Launch EPIC-CCN-001
echo "[$(date)] Launching EPIC-CCN-001"
screen -dmS p3-001 bash -l -c './scripts/wave4/_p3_001.sh 2>&1 | tee logs/phase3/EPIC-CCN-001.log'

# Wait 12 seconds
sleep 12

# Launch EPIC-CCN-002
echo "[$(date)] Launching EPIC-CCN-002"
screen -dmS p3-002 bash -l -c './scripts/wave4/_p3_002.sh 2>&1 | tee logs/phase3/EPIC-CCN-002.log'

echo "[$(date)] Test launch complete (2 epics)"
echo "Monitor with: screen -ls"
echo "Check files: ls docs/brain/EPIC-CCN-{001,002}/03-audit-report.md"
'''

test_path = output_dir / "launch_phase3_test.sh"
with open(test_path, 'w', newline='\n') as f:
    f.write(test_launcher)
os.chmod(test_path, 0o755)
print(f"Generated: {test_path}")

# Generate full launcher (all 80 epics)
full_launcher = '''#!/bin/bash
# Phase 3 Full Wave Launcher - All 80 Epics
# Generated: 2026-06-15
# Delay: Constant 12s (building-blocks method)

set -e

echo "[$(date)] Starting Phase 3 full wave launch (80 epics)"

# Create logs directory
mkdir -p logs/phase3

# Launch all 80 epics with constant 12s delay
for i in $(seq -f "%03g" 1 80); do
    EPIC="EPIC-CCN-${i}"
    
    echo "[$(date)] Launching ${EPIC} (delay: 12s)"
    
    # Launch in screen session
    screen -dmS p3-${i} bash -l -c \\
        "./scripts/wave4/_p3_${i}.sh 2>&1 | tee logs/phase3/${EPIC}.log"
    
    # Constant 12s delay
    sleep 12
done

echo "[$(date)] All 80 epics launched for Phase 3"
echo "Launch duration: 16 minutes (80 × 12s)"
echo "Monitor with: screen -ls | grep -c 'p3-'"
echo "Check files: ls docs/brain/EPIC-CCN-*/03-audit-report.md | wc -l"
'''

full_path = output_dir / "launch_phase3_all.sh"
with open(full_path, 'w', newline='\n') as f:
    f.write(full_launcher)
os.chmod(full_path, 0o755)
print(f"Generated: {full_path}")

print(f"\n✅ Phase 3 script generation complete!")
print(f"   - 80 epic scripts: _p3_001.sh through _p3_080.sh")
print(f"   - Test launcher: launch_phase3_test.sh")
print(f"   - Full launcher: launch_phase3_all.sh")
print(f"\nNext steps:")
print(f"   1. Review scripts with: diff scripts/wave4/_p2_001.sh scripts/wave4/_p3_001.sh")
print(f"   2. Upload to VM: gcloud compute scp scripts/wave4/_p3_*.sh scripts/wave4/launch_phase3_*.sh v12-test-golden-v2:~/universal-or-strategy/ --zone=us-central1-a")
print(f"   3. Run pilot test: gcloud compute ssh v12-test-golden-v2 --zone=us-central1-a --command='cd universal-or-strategy && ./scripts/wave4/launch_phase3_test.sh'")

# Made with Bob
