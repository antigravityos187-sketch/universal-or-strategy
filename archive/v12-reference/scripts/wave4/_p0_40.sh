...
mkdir -p docs/brain/EPIC-CCN-40
mkdir -p logs/phase0

cat > /tmp/phase0_msg_40.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-40.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='ProcessFollowerCancellationSafe')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='ProcessFollowerCancellationSafe')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='ProcessFollowerCancellationSafe')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-40/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-40/00-hotspots.md << 'EOF'
# Phase 0: Hotspot Analysis - EPIC-CCN-40
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-40/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-40/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-40",
  "method": "ProcessFollowerCancellationSafe",
  "file": "src/V12_002.Orders.Callbacks.AccountOrders.cs",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-40/00-hotspots.md && wc -l docs/brain/EPIC-CCN-40/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-40/manifest.json && cat docs/brain/EPIC-CCN-40/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
screen -dmS p0-40 bash -l -c "bob --yolo --chat-mode v12-phase0-hotspot \"$(cat /tmp/phase0_msg_40.txt)\" 2>&1 | tee logs/phase0/EPIC-CCN-40.log"
echo "DONE_EXIT=$?"
