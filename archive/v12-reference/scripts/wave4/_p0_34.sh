...
mkdir -p docs/brain/EPIC-CCN-34
mkdir -p logs/phase0

cat > /tmp/phase0_msg_34.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-34.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-34/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-34/00-hotspots.md << 'EOF'
# Phase 0: Hotspot Analysis - EPIC-CCN-34
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-34/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-34/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-34",
  "method": "ProcessQueuedAccountOrder",
  "file": "src/V12_002.Orders.Callbacks.AccountOrders.cs",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-34/00-hotspots.md && wc -l docs/brain/EPIC-CCN-34/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-34/manifest.json && cat docs/brain/EPIC-CCN-34/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
screen -dmS p0-34 bash -l -c "bob --yolo --chat-mode v12-phase0-hotspot \"$(cat /tmp/phase0_msg_34.txt)\" 2>&1 | tee logs/phase0/EPIC-CCN-34.log"
echo "DONE_EXIT=$?"
