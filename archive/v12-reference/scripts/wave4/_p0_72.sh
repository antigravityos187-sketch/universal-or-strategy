...
mkdir -p docs/brain/EPIC-CCN-72
mkdir -p logs/phase0

cat > /tmp/phase0_msg_72.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-CCN-72.
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-72/00-hotspots.md:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-72/00-hotspots.md << 'EOF'
# Phase 0: Hotspot Analysis - EPIC-CCN-72
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-CCN-72/manifest.json:

```xml
<execute_command>
<command>
cat > docs/brain/EPIC-CCN-72/manifest.json << 'EOF'
{
  "epic_id": "EPIC-CCN-72",
  "method": "SyncLimitTarget",
  "file": "src/V12_002.Orders.Management.StopSync.cs",
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-72/00-hotspots.md && wc -l docs/brain/EPIC-CCN-72/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
ls -lh docs/brain/EPIC-CCN-72/manifest.json && cat docs/brain/EPIC-CCN-72/manifest.json | head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
screen -dmS p0-72 bash -l -c "bob --yolo --chat-mode v12-phase0-hotspot \"$(cat /tmp/phase0_msg_72.txt)\" 2>&1 | tee logs/phase0/EPIC-CCN-72.log"
echo "DONE_EXIT=$?"
