#!/bin/bash
# Phase 4 (Ticket Generation) for EPIC-CCN-059
...
cat > /tmp/phase4_msg_059.txt << 'EOFMSG'
Use the phase-4-tickets MCP server to execute Phase 4 for EPIC-CCN-059.

Call the execute_phase_4 tool with epic_id="EPIC-CCN-059".
...
bob --yolo "$(cat /tmp/phase4_msg_059.txt)"

# Verify file created
if [ -f "docs/brain/EPIC-CCN-059/04-tickets.md" ]; then
    echo "SUCCESS: Phase 4 complete for EPIC-CCN-059"
    echo "File: docs/brain/EPIC-CCN-059/04-tickets.md"
    ls -lh "docs/brain/EPIC-CCN-059/04-tickets.md"
else
