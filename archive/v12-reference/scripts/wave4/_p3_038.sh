#!/bin/bash
# Phase 3 (DNA & PR Audit) for EPIC-CCN-038
...
cat > /tmp/phase3_msg_038.txt << 'EOFMSG'
Use the phase-3-audit MCP server to execute Phase 3 for EPIC-CCN-038.

Call the execute_phase_3 tool with epic_id="EPIC-CCN-038".
...
bob --yolo "$(cat /tmp/phase3_msg_038.txt)"

# Verify file created
if [ -f "docs/brain/EPIC-CCN-038/03-audit-report.md" ]; then
    echo "SUCCESS: Phase 3 complete for EPIC-CCN-038"
    echo "File: docs/brain/EPIC-CCN-038/03-audit-report.md"
    ls -lh "docs/brain/EPIC-CCN-038/03-audit-report.md"
else
