...
if ! find docs/brain/EPIC-CCN-060 -maxdepth 1 \( -name "05-*.md" -o -name "ticket-*-completion.md" -o -name "ticket-completion.md" \) -print -quit | grep -q .; then
...
if [ -f "docs/brain/EPIC-CCN-060/06-completion-report.md" ]; then
    ...
    echo "ERROR: No verification report created for EPIC-CCN-060"
    exit 1
fi
