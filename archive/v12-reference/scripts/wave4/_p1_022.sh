...
cat > /tmp/phase1_msg_022.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition + Boundary Validation) for EPIC-CCN-022.
...
**Input**: Read `docs/brain/EPIC-CCN-022/00-hotspots.md`
...
Create `docs/brain/EPIC-CCN-022/01-scope.md` with:
...
2. **Boundary Definition**:
   - What's IN scope: PropagateMaster_IdentifyMove body only
...
Create `docs/brain/EPIC-CCN-022/01-scope-boundary.md` with:

1. **Boundary Check**:
   - ✅ Scope limited to single method: PropagateMaster_IdentifyMove
   - ✅ No changes to callers
   - ✅ No changes to callees
   - ✅ No changes to other methods in V12_002.Orders.Callbacks.Propagation.cs
...
cat > docs/brain/EPIC-CCN-022/01-scope.md << 'EOF'
[Your scope definition content here]
EOF

# Create 01-scope-boundary.md
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat > docs/brain/EPIC-CCN-022/01-scope-boundary.md << 'EOF'
[Your boundary validation content here]
EOF

# Verify files
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-022/01-scope.md && wc -l docs/brain/EPIC-CCN-022/01-scope.md

execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
ls -lh docs/brain/EPIC-CCN-022/01-scope-boundary.md && wc -l docs/brain/EPIC-CCN-022/01-scope-boundary.md

# Update manifest
execute_command with cwd=/home/malhitticrypto/universal-or-strategy:
cat docs/brain/EPIC-CCN-022/manifest.json
```
...
bob --yolo --chat-mode plan "$(cat /tmp/phase1_msg_022.txt)" 2>&1 | tee logs/phase1/EPIC-CCN-022.log
echo "DONE_EXIT=$?"
