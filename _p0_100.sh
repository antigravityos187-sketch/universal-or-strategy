...
/usr/bin/mkdir -p docs/brain/EPIC-W7-100
/usr/bin/mkdir -p logs/phase0

/usr/bin/cat > /tmp/phase0_msg_100.txt << 'EOFMSG'
Execute Phase 0 (Hotspot Analysis) for EPIC-W7-100.
...
2. get_blast_radius(repo='universal-or-strategy', symbol='ClosePositionsOnlyApexAccounts')
3. get_call_hierarchy(repo='universal-or-strategy', symbol_id='ClosePositionsOnlyApexAccounts')
4. get_symbol_complexity(repo='universal-or-strategy', symbol_id='ClosePositionsOnlyApexAccounts')

### Step 2: Write 00-hotspots.md using execute_command
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-W7-100/00-hotspots.md:

```xml
<execute_command>
<command>
/usr/bin/cat > docs/brain/EPIC-W7-100/00-hotspots.md << 'EOF'
...
Use execute_command (NOT run_shell_command) to create docs/brain/EPIC-W7-100/manifest.json:

```xml
<execute_command>
<command>
/usr/bin/cat > docs/brain/EPIC-W7-100/manifest.json << 'EOF'
{
  "epic_id": "EPIC-W7-100",
  "method": "ClosePositionsOnlyApexAccounts",
  "file": "src/V12_002.SIMA.Flatten.cs",
...
<execute_command>
<command>
/usr/bin/ls -lh docs/brain/EPIC-W7-100/00-hotspots.md && wc -l docs/brain/EPIC-W7-100/00-hotspots.md</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
<execute_command>
<command>
/usr/bin/ls -lh docs/brain/EPIC-W7-100/manifest.json && cat docs/brain/EPIC-W7-100/manifest.json |/usr/bin/head -20</command>
<cwd>/home/malhitticrypto/universal-or-strategy</cwd>
</execute_command>
```
...
/home/malhitticrypto/.npm-global/bin/bob --yolo --chat-mode v12-phase0-hotspot "$(/usr/bin/cat /tmp/phase0_msg_100.txt)" 2>&1 |/usr/bin/tee logs/phase0/EPIC-W7-100.log
