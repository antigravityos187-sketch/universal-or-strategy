...
mkdir -p docs/brain/EPIC-W7-159
mkdir -p logs/wave7/phase1

if [ ! -f "docs/brain/EPIC-W7-159/00-hotspots.md" ]; then
    echo "BLOCKED: Phase 0 not complete"
    exit 1
fi

cat > /tmp/phase1_msg_159.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition) for EPIC-W7-159.
...
3. Write docs/brain/EPIC-W7-159/00-scope.md using execute_command
...
~/.npm-global/bin/bob --yolo --chat-mode v12-phase1-scope "$(cat /tmp/phase1_msg_159.txt)"
DONE_EXIT=$?
echo "Phase 1 complete for EPIC-W7-159 (exit: $DONE_EXIT)"
exit $DONE_EXIT
