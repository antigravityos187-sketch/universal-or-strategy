...
mkdir -p docs/brain/EPIC-W7-119
mkdir -p logs/wave7/phase1

if [ ! -f "docs/brain/EPIC-W7-119/00-hotspots.md" ]; then
    echo "BLOCKED: Phase 0 not complete"
    exit 1
fi

cat > /tmp/phase1_5_msg_001.txt << 'EOFMSG'
Execute Phase 1 (Scope Definition) for EPIC-W7-119.
...
Input: docs/brain/EPIC-W7-119/00-hotspots.md

Required Actions:
1. Read hotspot analysis
2. Define extraction scope (IN SCOPE vs OUT OF SCOPE)
3. Write docs/brain/EPIC-W7-119/01-scope-boundary.md using execute_command
...
bob --yolo --chat-mode v12-phase1-scope "$(cat /tmp/phase1_5_msg_001.txt)" 2>&1 | tee logs/wave7/phase1/EPIC-W7-119.log
echo "DONE_EXIT=$?"
